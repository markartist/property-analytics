...
async function readSnapshot<T>(name: string): Promise<T> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return fallbackSnapshot(name) as T;
    }
    throw error;
  }
}
...
function fallbackSnapshot(name: string):
  | OverviewSnapshot
  | CwvSnapshot
  | TrafficSnapshot
  | FunnelSnapshot
  ...
  switch (name) {
    case "overview.json":
      return { meta: emptyMeta(), sections: [] };
    case "cwv.json":
      return { meta: emptyMeta(), rollups: {}, pairs: [] };
    case "traffic.json":
      return { meta: emptyMeta(), metrics: [] };
    case "funnel.json":
      return { meta: emptyMeta(), metrics: [] };
    case "properties.json":
      return { pairs: [] };
    case "archive.json":
      return { latest: { date: "n/a", workbook_path: null, email_preview_path: null }, runs: [] };
    default:
      throw new Error(`Unsupported pilot KPI snapshot fallback: ${name}`);
  }
}
