...
const signalStyles: Record<SignalColor, { dot: string; badge: string; text: string; icon: React.ElementType }> = {
  ...
  yellow: {
    dot: "bg-[#BD4830]",
    badge: "border-[#BD4830]/35 bg-[#BD4830]/10 text-[#15284B]",
    text: "Yellow",
    icon: Clock3,
  },
  red: {
    dot: "bg-[#E02472]",
    badge: "border-[#E02472]/35 bg-[#E02472]/12 text-[#15284B]",
    text: "Red",
    icon: ShieldCheck,
  },
  gray: {
  ...
  return (
    <span
      className={cn(
        ...
        "shadow-[inset_0_1px_0_rgba(255,255,255,0.42),0_6px_18px_rgba(21,40,75,0.08)] backdrop-blur-md",
        style.badge,
      )}
    >
...
function ExternalUrl({ label, url }: { label: string; url: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex max-w-full items-center gap-1.5 break-all text-xs font-black leading-5 text-[#3D66B9] underline-offset-4 hover:underline"
    >
      {label}
...
function Dial({
  ...
  return (
    <div className={cn("resi-waterglass-stage-card rounded-lg border", size === "large" ? "p-4" : "p-3")}>
      <div className="flex items-center gap-3">
        ...
        >
          <div className={cn("grid place-items-center rounded-full bg-white/90 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.48)]", size === "large" ? "h-[72px] w-[72px]" : "h-12 w-12")}>
...
function PortfolioChart() {
  return (
    <div className="resi-waterglass-card rounded-lg border p-4">
...
function averageOptimizedScore(scoreKey: "mobileScore" | "desktopScore") {
  const scores = launchSnapshot.properties
    .map((property) => property.optimizedProof?.[scoreKey])
    .filter((score): score is number => typeof score === "number");
  if (!scores.length) return null;
  return Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length);
}

function optimizedMeasured() {
  return launchSnapshot.properties.filter((property) => {
    const proof = property.optimizedProof;
    return proof?.status === "captured" && typeof proof.mobileScore === "number" && typeof proof.desktopScore === "number";
  }).length;
}
...
function propertyDisplayNextStep(property: LaunchProperty) {
  if (!property.analyticsHygiene.clean) {
    return `Next: Resi removes duplicate Heap/debug output on ${domainFromUrl(property.newUrl.url)}, then WebOps reruns analytics hygiene proof.`;
  }
  if (propertyHasContentIssue(property)) {
...
function propertyPhases(property: LaunchProperty): LaunchPhase[] {
  ...
  return [
    ...
    },
    {
      ...
      value: property.analyticsHygiene.clean ? "Clean" : "Fix",
      percent: property.analyticsHygiene.clean ? 100 : 25,
      status: {
        color: property.analyticsHygiene.color,
        label: property.analyticsHygiene.label,
        detail: property.analyticsHygiene.clean
          ? "Production analytics is clean for optimization."
          : "Duplicate Heap/debug output must be removed before optimization.",
      },
    },
  ];
}
...
function portfolioPhases(): LaunchPhase[] {
  ...
  const analyticsClean = launchSnapshot.summary.analyticsHygieneClean;
  ...
  return [
    ...
    {
      label: "Analytics Hygiene",
      value: `${analyticsClean}/${total}`,
      percent: Math.round((analyticsClean / total) * 100),
      status: {
        color: analyticsClean === total ? "green" : "red",
        label: analyticsClean === total ? "Clean" : "Blocked",
        detail: analyticsClean === total ? "All live pages have one approved Heap path." : "Duplicate Heap/debug output is still present on live pages.",
      },
    },
  ];
}
...
function workAheadRows(): LaunchBreakdown[] {
  ...
  const primaryPending = launchSnapshot.properties.filter(propertyPrimaryPending).length;
  const contentIssues = launchSnapshot.properties.filter(propertyHasContentIssue).length;
  const analyticsBlocked = total - launchSnapshot.summary.analyticsHygieneClean;
  return [
    { label: "Analytics cleanup blockers", value: analyticsBlocked, tone: analyticsBlocked ? "red" : "green" },
...
function CommandSummary() {
  ...
  const rows = [
    ...
    {
      label: "Analytics",
      value: `${launchSnapshot.summary.analyticsHygieneClean}/${total}`,
      tone: launchSnapshot.summary.analyticsHygieneClean === total ? "green" as SignalColor : "red" as SignalColor,
      detail: launchSnapshot.summary.analyticsHygieneClean === total ? "Clean" : "Cleanup",
    },
    {
      label: "Hero Media",
      value: launchSnapshot.summary.heroFreshnessChecked ? `${launchSnapshot.summary.heroFreshnessCurrent}/${total}` : "Pending",
      tone:
        launchSnapshot.summary.heroFreshnessChecked &&
        launchSnapshot.summary.heroFreshnessRefreshNeeded + launchSnapshot.summary.heroFreshnessSourceMissing + launchSnapshot.summary.heroFreshnessSourceError === 0
          ? "green" as SignalColor
          : "yellow" as SignalColor,
      detail: launchSnapshot.summary.heroFreshnessRefreshNeeded ? `${launchSnapshot.summary.heroFreshnessRefreshNeeded} refresh` : "Freshness",
    },
  ...
  return (
    <div className="mt-5 grid gap-3 xl:grid-cols-[1fr_320px]">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
        {rows.map((row) => (
          <div key={row.label} className="resi-waterglass-stage-card rounded-lg border p-3">
      ...
      <div className="resi-waterglass-dark rounded-lg border p-4 text-white">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <span className="rounded-full bg-[#E02472] px-3 py-1.5 text-sm font-black text-white">{total - launchSnapshot.summary.analyticsHygieneClean} analytics blockers</span>
          <span className="rounded-full bg-[#BD4830] px-3 py-1.5 text-sm font-black text-white">{launchSnapshot.summary.vanityQaCorePageIssues} core page item</span>
        </div>
...
function PhaseCard({ phase, index }: { phase: LaunchPhase; index: number }) {
  return (
    <div className="resi-waterglass-card relative rounded-lg border p-4">
...
function PortfolioPipeline() {
  ...
  return (
    <section className="resi-waterglass-card mb-4 rounded-lg border p-4">
...
function BenchmarkSummary() {
  ...
  const optimizedMobile = averageOptimizedScore("mobileScore");
  const optimizedDesktop = averageOptimizedScore("desktopScore");
  const total = launchSnapshot.summary.totalProperties;
  const legacyMeasured = measuredTargets(0);
  const kinstaMeasured = measuredTargets(1);
  const vanityMeasured = measuredTargets(2);
  const optimizedCount = optimizedMeasured();
  ...
  return (
    <section className="resi-waterglass-card mb-4 rounded-lg border p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        />
        <StageAverageCard
          eyebrow={optimizedCount ? "Current Proof" : "Future State"}
          title="Optimized"
          mobile={optimizedCount ? optimizedMobile : "90+"}
          desktop={optimizedCount ? optimizedDesktop : "95+"}
          measured={optimizedCount ? `${optimizedCount}/${total}` : "Target"}
          helper={optimizedCount ? "Governed optimization proof captured." : "Post-optimization target lane for governed proof."}
          target={!optimizedCount}
        />
      </div>
    </section>
  );
    ...
    <div className="resi-waterglass-card rounded-lg border p-4">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
        {propertyPhases(property).map((phase, index) => (
          <div key={phase.label} className="resi-waterglass-stage-card rounded-lg border p-3">
            <div className="flex items-center justify-between gap-2">
    ...
    <section className="mb-4 grid gap-3 xl:grid-cols-[1.15fr_0.85fr]">
      <div className="resi-waterglass-card rounded-lg border p-4">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="resi-waterglass-stage-card rounded-lg border p-3">
...
function StageAverageCard({
  ...
  return (
    <div className="resi-waterglass-stage-card rounded-lg border p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
...
function StageScoreBlock({ label, score, target = false }: { label: string; score: ScoreValue; target?: boolean }) {
  ...
  return (
    <div className="resi-waterglass-card min-w-[116px] rounded-lg border p-3">
      <div className="flex items-center justify-between gap-2">
...
function MoveStageCard({ stage, index }: { stage: PerformanceStage; index: number }) {
  return (
    <div className="resi-waterglass-stage-card min-w-0 rounded-lg border p-4">
...
function CompactStageCell({ stage, index }: { stage: PerformanceStage; index: number }) {
  const mobileTone = scoreValueTone(stage.mobileScore, stage.target);
  const desktopTone = scoreValueTone(stage.desktopScore, stage.target);
  const pendingTarget = stage.title === "Optimized" && stage.target;

  return (
    <div
      className={cn(
        "resi-waterglass-stage-card relative min-w-0 overflow-hidden rounded-lg border p-2.5",
        pendingTarget && "opacity-70",
      )}
    >
      {pendingTarget ? (
        <div className="pointer-events-none absolute inset-x-2.5 top-2.5 z-10 rounded-full border border-[#D6D6D2] bg-white/92 px-2 py-0.5 text-center text-[9px] font-black uppercase tracking-[0.12em] text-[#9B9B96] shadow-sm">
          Pending
        </div>
      ) : null}
      <div className={cn("flex items-start justify-between gap-2", pendingTarget && "pt-6")}>
        <div className="flex min-w-0 items-start gap-2">
          <span
            className="grid h-6 w-6 shrink-0 place-items-center rounded-full text-[11px] font-black text-white"
            style={{ background: toneColors[stage.signal.color] }}
          >
            {index + 1}
          </span>
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase text-[#9B9B96]">{stage.eyebrow}</p>
            <p className="mt-0.5 text-sm font-black leading-5 text-[#15284B]">{stage.title}</p>
          </div>
        </div>
        <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: toneColors[stage.signal.color] }} />
      </div>
      <div className="mt-2 min-h-[34px]">
        <ExternalUrl label={stage.url.replace("https://", "")} url={stage.url} />
      </div>
      <div className="mt-2 grid grid-cols-2 gap-1.5">
        <div className="rounded-md border border-[#D6D6D2]/80 bg-white/84 px-2 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.42)]">
          <div className="flex items-center justify-between gap-1">
            <span className="text-[10px] font-black uppercase text-[#9B9B96]">M</span>
            <span className="h-2 w-2 rounded-full" style={{ background: toneColors[mobileTone] }} />
          </div>
          <p className={cn("mt-0.5 text-base font-black leading-5", stage.target ? "text-[#9B9B96]" : "text-[#15284B]")}>
            {stage.mobileScore ?? "-"}
          </p>
        </div>
        <div className="rounded-md border border-[#D6D6D2]/80 bg-white/84 px-2 py-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.42)]">
          <div className="flex items-center justify-between gap-1">
            <span className="text-[10px] font-black uppercase text-[#9B9B96]">D</span>
            <span className="h-2 w-2 rounded-full" style={{ background: toneColors[desktopTone] }} />
          </div>
          <p className={cn("mt-0.5 text-base font-black leading-5", stage.target ? "text-[#9B9B96]" : "text-[#15284B]")}>
            {stage.desktopScore ?? "-"}
          </p>
...
function CompactStageTrack({ stages }: { stages: PerformanceStage[] }) {
  return (
    <div className="grid min-w-0 gap-2 md:grid-cols-2 xl:grid-cols-[1fr_22px_1fr_22px_1fr_22px_1fr] xl:items-stretch">
      {stages.map((stage, index) => (
        <React.Fragment key={stage.title}>
          <CompactStageCell stage={stage} index={index} />
          {index < stages.length - 1 ? (
            <div className="hidden items-center justify-center xl:flex">
              <ArrowRight className="h-4 w-4 text-[#3D66B9]" />
            </div>
          ) : null}
        </React.Fragment>
      ))}
...
function performanceStages(property: LaunchProperty): PerformanceStage[] {
  ...
  const optimized = property.optimizedProof;
  const optimizedCaptured = optimized.status === "captured";
  ...
  return [
    ...
    {
      title: "Optimized",
      eyebrow: optimizedCaptured ? "Current Proof" : "Future State",
      url: optimized.url || property.newUrl.url,
      signal: optimizedCaptured
        ? { color: "green", label: "Captured", detail: optimized.note }
        : property.analyticsHygiene.clean
          ? { color: "gray", label: "Queued", detail: "Optimization begins after post-launch issues and promo rendering are stable." }
          : { color: "red", label: "Blocked", detail: "Analytics cleanup is required before optimization proof." },
      mobileScore: optimizedCaptured ? optimized.mobileScore : "90+",
      desktopScore: optimizedCaptured ? optimized.desktopScore : "95+",
      note: optimizedCaptured
        ? `Live optimization proof passed${optimized.runtimeVersion ? ` on ${optimized.runtimeVersion}` : ""}.`
        : property.analyticsHygiene.clean
          ? "Target lane for the governed optimization package and post-work proof."
          : "Waiting for duplicate Heap/debug cleanup before optimization begins.",
      target: !optimizedCaptured,
    },
  ];
}
          ...
          return (
            <div key={step.number} className="grid grid-cols-[32px_1fr] gap-3">
              ...
              <div className={cn("pb-4", isLast && "pb-0")}>
                <div className="resi-waterglass-stage-card flex flex-wrap items-center justify-between gap-2 rounded-lg border p-3">
                  <div>
...
function FactRows({ property }: { property: LaunchProperty }) {
  const iconByLabel: Record<string, React.ElementType> = {
    ...
    "Analytics hygiene": AlertTriangle,
    "Performance baseline": Gauge,
    "Hero media freshness": ImageIcon,
    ...
    <div className="grid gap-4 xl:grid-cols-2">
      <div className="resi-waterglass-card rounded-lg border p-4">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <div className="resi-waterglass-stage-card rounded-lg border p-2">
...
function PropertyDrawer({ property, canExpand }: { property: LaunchProperty; canExpand: boolean }) {
  const displaySignal = propertyDisplaySignal(property);
  const displayNextStep = propertyDisplayNextStep(property);
  const stages = performanceStages(property);
  const optimizedPending = property.optimizedProof.status !== "captured";

  const rowContent = (
    <div className={cn("grid gap-3", optimizedPending && "opacity-55 grayscale")}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-lg font-black leading-6 text-[#15284B]">{property.propertyName}</h2>
            <span className="rounded-full border border-[#D6D6D2] bg-[#F6F6F5] px-2.5 py-1 text-xs font-black text-[#294782]">
              {property.propertyCode}
            </span>
            <span className="text-sm font-bold text-[#294782]">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            <SignalBadge signal={property.analyticsHygiene} compact />
            <SignalBadge signal={property.heroFreshness} compact />
          </div>
        </div>
        {canExpand ? (
      ...
      <CompactStageTrack stages={stages} />
    </div>
  );
  ...
  if (!canExpand) {
    return (
      <div
        className={cn(
          "resi-waterglass-card relative overflow-hidden rounded-lg border p-3 sm:p-4",
          optimizedPending && "resi-waterglass-pending-row",
        )}
      >
        {optimizedPending ? (
          <div className="pointer-events-none absolute right-5 top-0 z-10 rounded-b-md border-x border-b border-[#D6D6D2] bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-[#9B9B96] shadow-sm">
  ...
  return (
    <details
      className={cn(
        "resi-waterglass-card group relative overflow-hidden rounded-lg border",
        optimizedPending && "resi-waterglass-pending-row",
      )}
    >
      {optimizedPending ? (
        <div className="pointer-events-none absolute right-14 top-0 z-10 rounded-b-md border-x border-b border-[#D6D6D2] bg-white px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-[#9B9B96] shadow-sm">
          Pending Optimization
        </div>
      ) : null}
...
export function LaunchDashboardClient() {
  ...
  const sortedProperties = React.useMemo(
    () => [...launchSnapshot.properties].sort((a, b) => a.propertyName.localeCompare(b.propertyName)),
    [],
  );

  return (
    <div className="resi-waterglass-stage min-h-screen text-[#15284B]">
      <div className="mx-auto max-w-6xl px-3 py-4 sm:px-4 lg:px-5">
        <header className="resi-waterglass-panel rounded-lg border p-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-black text-[#3D66B9]">Resi Edge Portfolio Launch</p>
              <h1 className="mt-1 text-2xl font-black text-[#15284B]">Property Move Monitor</h1>
        ...
        <main className="resi-waterglass-panel mt-5 rounded-lg border p-3 sm:p-4">
          <section className="mb-4">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-sm font-black uppercase tracking-[0.12em] text-[#3D66B9]">Property Results</p>
                <h2 className="mt-1 text-xl font-black text-[#15284B]">Legacy to Optimized Proof Path</h2>
              </div>
              <span className="rounded-full border border-[#D6D6D2] bg-white px-3 py-1.5 text-sm font-black text-[#15284B]">
                {summary.optimizedProofProperties}/{summary.totalProperties} optimized
              </span>
            </div>
            <div className="grid grid-cols-1 gap-3">
              {sortedProperties.map((property) => (
                <PropertyDrawer key={property.propertyCode} property={property} canExpand={isSteward} />
              ))}
            </div>
          </section>
          ...
          {isSteward ? (
            <>
              <div className="resi-waterglass-dark mb-4 rounded-lg border p-4 text-white">
                <div className="grid gap-3">
                  <div>
                    ...
                    <p className="mt-1 max-w-5xl text-xl font-black leading-8">
                      Expanded vanity QA: {summary.vanityQaGreen} green, {summary.vanityQaYellow} yellow, {summary.vanityQaRed} red. Public move is complete; optimization is waiting on analytics cleanup.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <TruthChip tone={summary.analyticsHygieneClean === summary.totalProperties ? "green" : "red"} label={`${summary.analyticsHygieneClean}/${summary.totalProperties} analytics clean`} />
