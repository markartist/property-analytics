...
def _source_freshness(conn: sqlite3.Connection) -> List[Dict[str, str]]:
    ...
    for source, sql, is_critical, source_key in checks:
        ...
        if source_key == "guest_cards":
            ...
            status = (
                "fresh" if expectation.status == "fresh"
                else "warning" if expectation.status == "warning"
                else "stale" if expectation.status == "stale"
                else "unknown"
            )
        else:
            age_text, status = _age_status(latest, critical=is_critical)
        rows.append(
            {
                "source": source,
                "latest": latest or "N/A",
...
def _latest_mirror_report() -> Dict[str, object]:
    ...
    try:
        ...
        return {"found": True, "file": str(report_path), "parse_error": True}
