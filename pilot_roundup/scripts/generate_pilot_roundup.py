...
def main() -> int:
    ...
    report_display_date = datetime.strptime(report_date, "%Y-%m-%d").strftime("%m-%d-%Y")
    ...
    paired_properties: List[Tuple[Dict[str, object], Dict[str, object]]] = []
    for pilot in entries:
        ...
        if sister_entry:
            ...
            paired_properties.append((pilot_payload, sister_payload))
    conn.close()
    ...
    executive_bullets = [
        ...
        f"{strongest_traffic['property_name']} leads pilot new-user volume with {fmt_int(strongest_traffic['new_users_today'])} new users on {ga_date}.",
    ...
    if not gt_date:
        executive_bullets.append(f"Fresh GTMetrix was unavailable for {report_date}, so today's GT values are shown as -.")
    if riskiest:
    ...
    overview_html = render_kpi_row(overview_tiles, columns=4) + render_kpi_row(sister_tiles, columns=4)

    performers_html = '<div style="margin-bottom: 25px;">'
    for pilot_payload, sister_payload in paired_properties:
    ...
    builder.add_section(Section(
        ...
        status=None,
        description="PSI mobile is the primary displayed metric. Each pilot is grouped with its matched sister/control property, while GTMetrix, New Users, and BrowserStack remain supporting context.",
    ))
    builder.add_section(Section(
    ...
    md_lines = [
        ...
        f"- PSI date: {psi_date or '—'}",
        f"- GTMetrix date: {gt_date or '—'}",
        f"- GA4 date: {ga_date or '—'}",
        "",
        "## Executive Summary",
    ]
