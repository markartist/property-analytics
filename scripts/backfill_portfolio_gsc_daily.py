#!/usr/bin/env python3
"""Backfill portfolio GSC daily performance rows for an executive report window."""
...
def fetch_daily_rows(service: Any, *, site_url: str, start: date, end: date) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        parsed_rows.append(
            (
                ...
                float(row.get("ctr", 0) or 0) * 100,
                float(row.get("position", 0) or 0),
                gsc_site_url if gsc_site_url.endswith("/") else f"{gsc_site_url}/",
                ga4_property_id,
            )
        )
    if not parsed_rows:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backfill GSC daily rows for the portfolio.")
    parser.add_argument("--start-date", required=True, help="Start date, YYYY-MM-DD")
    parser.add_argument("--end-date", required=True, help="End date, YYYY-MM-DD")
    parser.add_argument("--chunk-days", type=int, default=92, help="Days per API query chunk")
    ...
    if args.chunk_days < 1 or args.chunk_days > 366:
        raise RuntimeError("--chunk-days must be between 1 and 366")

    run_id = datetime.now().strftime("%Y%m%d_%H%M%S_gsc_daily")
    ...
    with sqlite3.connect(DB_PATH) as conn:
        (run_dir / "coverage_before.json").write_text(
            json.dumps(table_coverage(conn), indent=2, ensure_ascii=True) + "\n",
            encoding="utf-8",
        )
    ...
    collection_id = None
    if not args.dry_run:
        collection_id = db.start_data_collection(date.today(), "backfill", "gsc_daily_metrics")
    ...
    print(f"GSC daily backfill: {start.isoformat()} through {end.isoformat()}")
    ...
    for index, prop in enumerate(props, start=1):
        ...
        for chunk_start, chunk_end in chunks:
            try:
                rows = fetch_daily_rows(collector.service, site_url=prop.gsc_url, start=chunk_start, end=chunk_end)
                prop_returned += len(rows)
                rows_returned += len(rows)
                if rows and not args.dry_run:
                    written = write_daily_rows(
                        DB_PATH,
        ...
        if status == "failed":
            ...
            skipped += 1
        property_results.append({**asdict(prop), "status": status, "rows_returned": prop_returned, "rows_written": prop_written, "errors": prop_errors})
    ...
    (run_dir / "backfill_summary.json").write_text(json.dumps(summary, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")
    with sqlite3.connect(DB_PATH) as conn:
    ...
    if collection_id is not None:
        error_message = "; ".join(f"{row['name']}: {row['errors'][0]}" for row in property_results if row["errors"])[:400] or None
        db.complete_data_collection(
            ...
            notes=f"Andrew Foresi executive search report GSC daily backfill; rows_written={rows_written}.",
        )
