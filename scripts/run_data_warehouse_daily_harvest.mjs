...
function keeperPassword(recordTitle) {
  ...
  if (!Array.isArray(list) || list.length === 0) {
    throw new Error(`Keeper record not found for ${recordTitle}.`);
  }
  ...
  if (!password) {
    throw new Error(`Keeper record ${recordTitle} does not contain a password value.`);
  }
  return password;
}
    ...
    for (const row of freshnessRows) {
      ...
      if (serverTime && maxDt && maxDt.getTime() > serverTime.getTime() + 24 * 60 * 60 * 1000) {
        dataQualityItems.push({
          severity: "review",
          check_type: "future_dated_source_max_timestamp",
          source: row.source,
          detail: `Latest source timestamp ${maxDt.toISOString()} is more than 24 hours after SQL Server time ${serverTime.toISOString()}.`,
        });
      }
    }
