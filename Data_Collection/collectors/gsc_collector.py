...
class GoogleSearchConsoleCollector:
    ...
    def _initialize_gsc_service(self):
        ...
        try:
            ...
            oauth_creds_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'client_secret_911627664995-s8derelblr6nfpf7hg8di7bs338jica5.apps.googleusercontent.com.json')
            token_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'gsc_token.pickle')
            
            if os.path.exists(oauth_creds_path):
                credentials = self._get_oauth_credentials(oauth_creds_path, token_path)
                if credentials:
