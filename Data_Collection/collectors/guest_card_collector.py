...
portfolio_analytics.db, then archives processed CSV files.
"""
...
class GuestCardCollector:
    ...
    def _archive_file(self, file_path: Path) -> None:
        ...
        # Retry move for transient OneDrive lock/deadlock behavior.
    ...
    def _read_csv_rows_with_retry(self, csv_file: Path, attempts: int = 10, sleep_seconds: float = 1.0) -> List[Dict[str, object]]:
        """Read CSV rows with retries for transient OneDrive lock errors."""
        last_err: Optional[Exception] = None
        for _ in range(attempts):
            try:
                with open(csv_file, "r", encoding="utf-8-sig", newline="") as f:
                    reader = csv.DictReader(f)
                    return [self._clean_headers(row) for row in reader]
            except OSError as e:
                last_err = e
                if e.errno not in (errno.EAGAIN, errno.EWOULDBLOCK, 35):
                    raise
        ...
        try:
            ...
            for csv_file in pending_files:
                try:
                    ...
                    rows = self._read_csv_rows_with_retry(csv_file)
                    for row in rows:
