...
class AnomalyDetector:
    ...
    # Metrics to monitor
    GA4_METRICS = ['sessions', 'users', 'pageviews', 'new_users']
    ...
    def _calculate_new_users_baseline(self, cursor: sqlite3.Cursor, property_id: str,
                                      start_date: date, end_date: date) -> Optional[float]:
        """Calculate a rolling new_users baseline without requiring schema changes."""
        cursor.execute("""
            SELECT AVG(new_users)
            FROM ga4_daily_metrics
            WHERE property_id = ?
              AND metric_date BETWEEN ? AND ?
        """, (property_id, start_date, end_date))
        ...
        with self.db.get_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                try:
                    ...
                    # Track new_users via rolling baseline computed on demand.
                    new_users_baseline = self._calculate_new_users_baseline(
                        cursor,
                        prop_id,
                        metric_date - timedelta(days=self.BASELINE_WINDOW_DAYS),
                        metric_date - timedelta(days=1),
                    )
                    if new_users_baseline is not None:
                        baselines['new_users'] = new_users_baseline
    ...
    def _get_actual_values(self, cursor: sqlite3.Cursor, property_id: str,
        ...
        # Get GA4 actuals
        cursor.execute("""
            SELECT sessions, total_users, pageviews, new_users
            FROM ga4_daily_metrics
        ...
        if row:
            ...
            actuals['new_users'] = row[3] or 0.0
