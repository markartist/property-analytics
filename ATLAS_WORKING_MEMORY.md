...
- Data Warehouse replacement tracks for guest cards, operating metrics, property metadata/region identity, and manual CSV seed paths are now documented in `/Users/mark/Property_Analytics/docs/DATA_WAREHOUSE_REPLACEMENT_TRACKS_2026-05-26.md`; `/Users/mark/Property_Analytics/apps/api/scripts/guest_cards_to_d1.py` now reads a unified guest-card source that prefers `guest_card_metrics_dw_direct` by date/property and falls back to `guest_card_metrics`, `/Users/mark/Property_Analytics/scripts/supply_property_operating_metrics_from_data_warehouse.mjs` supplies `property_operating_metrics` from `dw_read.leasingstatistics_daily_bv` using the latest available DW operating date, `/Users/mark/Property_Analytics/scripts/supply_property_metadata_from_data_warehouse.mjs` writes `property_metadata_dw_direct` from `dw_read.property_bv`, and `/Users/mark/Property_Analytics/scripts/audit_manual_source_replacements.mjs` audits `/Users/mark/Property_Analytics/config/manual_source_replacement_manifest.json`; latest proof supplied 91 operating-metric rows for effective date `2026-05-25`, 92 metadata rows for `2026-05-26`, and a manual-source audit with 6 candidates and 0 missing replacement paths; the four operating-stat-only DW codes `FL4CA`, `FL4P9`, `TX4FP`, and `TX4PW` are resolved through `/Users/mark/Property_Analytics/config/data_warehouse_property_code_resolution.json` as governed exclusions, while user-confirmed pre-live properties `TX4EK` / The Vine Kyle Parkway and `TX4CY` / Sundara at Spring Cypress are excluded from expected DW operating counts until live
- Watchtower now exposes a `data_warehouse_replacements` block from `/Users/mark/Property_Analytics/apps/api/src/routes/health.ts` and a DW replacement card strip in `/Users/mark/Property_Analytics/apps/web/src/app/watchtower/page.tsx`; `/Users/mark/Property_Analytics/scripts/generate_data_warehouse_replacement_review.mjs` creates repeatable review packets under `/Users/mark/Property_Analytics/outputs/data_warehouse/replacement_reviews/`, with latest packet `/Users/mark/Property_Analytics/outputs/data_warehouse/replacement_reviews/20260526_201454/data_warehouse_replacement_review.md` showing the lifecycle count check matches the expected live operating population
- The Andrew web platform architecture research packet is archived for future reference at `/Users/mark/Property_Analytics/docs/archive/research/andrew_web_platform_architecture_2026-05-26/`; it preserves the Andrew-facing report, internal web platform notes, SSL notes, Resi platform notes, and a local DOCX renderer, while generated DOCX/PDF/page-render output remains ignored and reproducible rather than source-controlled
