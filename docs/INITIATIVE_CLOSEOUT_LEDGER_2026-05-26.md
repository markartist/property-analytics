...
| Pilot CWV / tracker / roundup reporting | Partially promoted - reporting ops / tracker parked | `ff7ec1a`, `codex/closeout-pilot-reporting-2026-05-27`, `codex/closeout-pilot-tracker-2026-05-27`, `pilot_control_cwv/`, `pilot_roundup/`, nested `apps/pilot-tracker-standalone/` | Pilot reporting operations are split and promoted; the main web tracker route is absent on the clean branch, so broken `/tracker` navigation is hidden and the standalone nested tracker remains ignored/parked rather than absorbed into the parent repo | Promote reporting ops / park tracker product lane | Python compile, shell syntax, GTMetrix cohort validation, email dry-runs, web typecheck, context discipline, and PIB guardrails pass | Revisit tracker only as an explicit product migration from the nested standalone repo into `apps/web` |
...
| 2026-05-27 | Park Pilot tracker product lane on the parent repo closeout path | The clean parent branch has no main-app `/tracker` route, while the standalone tracker is an ignored nested Git repo with generated static/data artifacts and dirty internal state | Hide broken parent navigation; do not absorb nested tracker artifacts until a dedicated product migration is approved |
...
3. Treat Pilot tracker product work as parked until a dedicated migration from `apps/pilot-tracker-standalone` into `apps/web` is explicitly approved.
