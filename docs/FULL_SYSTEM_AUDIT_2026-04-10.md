...
09/04/2026 Cloudflare scheduled-lane activation addendum: Mark approved proceeding with the alive-stage activation for scheduled/read-model lanes while keeping individual Resi Edge property Workers, DNS, WordPress/Kinsta, Jira, Confluence, Microsoft 365, and locked PIB files out of scope. Remote D1 migrations `0067_create_agent_readiness_tables.sql` and `0068_create_captain_routine_schedule.sql` were applied to `pop-brief-db`. Agent Readiness target seeding now has `123` active targets, split across `95` corporate property pages and `28` Resi vanity targets. `pop-brief-api` was deployed as Worker version `beab4f92-3b1a-4c03-9078-c75422b39909`; its `*/15` Cron schedule now carries Captain routine execution, PIB reports, Resi Edge promo sync, and Resi Edge hero freshness sync as independent lanes. Production schedule readback showed `996` active Captain routine rows, `38` leased rows, no due backlog, and recent scheduled Captain runs writing as expected. The Agent Readiness Worker was redeployed as version `9a42a512-ff3f-4656-a8f9-52d8d2e56eea` after a success-insert placeholder mismatch was fixed and covered by `/Users/mark/Property_Analytics/tests/test_agent_readiness_worker_sql.py`; prior current results remain failed until the next six-hour scheduled scan, and manual runs remain closed until `AGENT_READINESS_ADMIN_SECRET` is represented in Keeper/KSM. Resi Edge hero freshness was corrected to normalize same-origin `/__resi-edge/native-dam-asset?src=...` proxy URLs back to the underlying DAM URL before comparison and hashing; production proof `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-freshness-manual-sync/20260904T202716Z/summary.json` returned `27/27` current, `0` refresh needed, `0` source missing, and `0` source errors. The manual promo fallback now writes remote R2 with `--remote`; production proof `/Users/mark/Property_Analytics/reports/resi_edge_performance/promo-record-sync/20260904T202046Z/summary.json` uploaded `27/27` promo records with `16` active specials and `0` missing feed rows. Resi Edge hero media refresh Worker version `4861d95e-d405-4e5f-9b5e-d44a859b646e` and central topper service version `26e3c8dd-ff65-4bc8-bf86-0075d857f8e1` are deployed, with media queue execution still disabled by design. The Pond web bundle was published to Cloudflare Pages deployment `https://aef5fb82.property-analytics.pages.dev`; smoke checks returned `200` for `/pond`, `/commodores`, `/captains/AR4PB`, `/site-content`, `/vacs`, and `/watchtower`, while `https://pond.venterrawebops.com/pond` returned the expected Cloudflare Access `302`. Validation passed with API platform tests `159/159`, API typecheck, web build, focused Python tests, and compile checks for touched scripts.

09/04/2026 Regional Commodore roster and standing orders addendum: Mark confirmed the regional layer needs named Commodores with active orders. Added `/Users/mark/Property_Analytics/config/commodore_roster.json` as the governed source for Commodore persona names, region assignments, call signs, activation status, standing-order status, review cadence, standing orders, and optional future human owner mapping. The first active roster stands up `16` system-persona Commodores aligned to governed regions from `/Users/mark/Property_Analytics/config/property_identity_matrix.json`. `/Users/mark/Property_Analytics/scripts/build_ops_watch_pond_snapshot.py` now emits roster metadata, `activeCommodoreCount`, and per-region identity/orders inside `commodoreBridge`; `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/types.ts` carries the updated typed contract; `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/generated-snapshot.ts` was regenerated; and `/Users/mark/Property_Analytics/apps/web/src/app/commodores/page.tsx` now renders a Commodore roster strip plus collapsible standing orders on regional cards. The operating model in `/Users/mark/Property_Analytics/docs/COMMODORE_BRIDGE_OPERATING_MODEL_2026-09-04.md` now documents the active roster and source-of-truth boundary. Boundary: read-only persona/roster/Bridge wiring only; no actual human regional manager assignments, Jira/Confluence/Microsoft 365/D1/R2/Captain Runtime/Cloudflare source mutations, source tickets, Resi content, or locked PIB files were changed.

09/04/2026 Commodore Bridge regional Captain layer addendum: Mark approved regional managers as the Commodore layer above property Captains. Commodores assess Captain signals across regional/cohort groups, identify cross-property trends and local nuance, share common knowledge, and package decision-ready escalation candidates for higher authority without becoming super-Captains. Added `/Users/mark/Property_Analytics/docs/COMMODORE_BRIDGE_OPERATING_MODEL_2026-09-04.md` as the governed operating model. `/Users/mark/Property_Analytics/scripts/build_ops_watch_pond_snapshot.py` now emits `commodoreBridge` in the generated Pond snapshot by joining Ops Watch/Ticket Care records to governed regions from `/Users/mark/Property_Analytics/config/property_identity_matrix.json`. `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/types.ts` carries the typed contract, and `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/generated-snapshot.ts` was regenerated from `/Users/mark/Property_Analytics/reports/ops_watch/ops-watch-20260904-0807/ops-watch-packet.json`. `/Users/mark/Property_Analytics/apps/web/src/app/commodores/page.tsx` adds the read-only Pond route `/commodores`; `/Users/mark/Property_Analytics/apps/web/src/lib/permissions.ts`, the sidebar, and the Pond landing page now expose `Commodore's Bridge` to editor/admin users. Current generated bridge snapshot has `94` active properties across `16` regions, `8` signaled properties, `8` active Jira ticket records, `2` regional escalation candidates, and `3` cross-region pattern candidates. Boundary: read-only regional intelligence, UI wiring, and documentation only; no Jira, Confluence, Microsoft 365, D1/R2, Captain Runtime, Cloudflare source state, source tickets, Resi content, or locked PIB mutation occurred.

09/04/2026 Captain Ticket Care and Ticket Wall addendum: Mark directed that Captains should care about property Jira tickets, raise flags, and champion follow-through rather than displaying a passive ticket list. Added `/Users/mark/Property_Analytics/docs/CAPTAIN_TICKET_CARE_SOP_2026-09-04.md` as the governed operating pattern. `/Users/mark/Property_Analytics/scripts/build_ops_watch_pond_snapshot.py` now emits `ticketCare` in the generated Pond Ops Watch snapshot, including per-ticket flags, blocker owner, evidence requirements, customer-promise guidance, recommended action, Captain stance, property queues, and portfolio patterns. `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/types.ts` carries the typed contract, and `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/generated-snapshot.ts` was regenerated from `/Users/mark/Property_Analytics/reports/ops_watch/ops-watch-20260904-0807/ops-watch-packet.json`. `/Users/mark/Property_Analytics/apps/web/src/app/captains/captain-office-client.tsx` now exposes a `Ticket wall` workspace that distinguishes source-system Jira stewardship from internal Captain Runtime watch items and presents progressive disclosure: status pressure, Captain read, action discipline, pattern cards, and compact ticket details. Published Cloudflare Pages deployment `https://63acf45d.property-analytics.pages.dev`; protected route remains `https://pond.venterrawebops.com/captains/<property>`. Current generated ticket-care snapshot has `8` Jira Captain records across `8` properties, including `8` Critical, `7` pending-vendor, `3` stale `14+` day, and `7` proof-needed records. Boundary: read-only snapshot enrichment, UI wiring, deployment, and documentation only; no Jira, Confluence, Microsoft 365, D1/R2, Captain Runtime, Resi content, source ticket, or locked PIB mutation occurred.

09/03/2026 Jira assisted closeout SOP addendum: Mark corrected the customer-facing proof-ticket closure pattern after a Whitney banner verification exposed Jira rich-editor duplication when text and pasted media were combined in a reply. `/Users/mark/Property_Analytics/docs/JIRA_CAPTAIN_WATCH_RUNBOOK_2026-08-22.md` now requires visual proof tickets to post the screenshot/image alone in `Reply to customer`, then place the personalized completion note only in the Done/Closed workflow transition comment. The required close remains `Thanks, and have a great day!`. Boundary: documentation/SOP only; no Jira tickets, Captain Runtime, Cloudflare, D1/R2, Confluence, Microsoft 365, Resi content, or locked PIB files were mutated by this addendum.

09/02/2026 Resi Edge one-command pre-apply gate bundle addendum: Mark approved adding a tighter operator flow before live apply. `/Users/mark/Property_Analytics/scripts/run_resi_edge_preapply_gates.py` now resolves one exact active manifest from `--property-code` and `--domain`, runs static package validation, gate coverage, named-property process scenario audit, batch inventory audit in inventory mode, central-topper local proof refresh, desktop-native visual gate refresh, deploy-bundle dry run through the deploy adapter, and read-only active scope-lock status. It writes consolidated evidence to `/Users/mark/Property_Analytics/reports/resi_edge_performance/preapply-gates/<domain>/<run-id>/preapply-gates.json` plus command records and a Markdown readout. The wrapper intentionally does not create the scope lock; missing, inactive, expired, or mismatched lock remains a blocker because live apply authorization must stay explicit. Boundary: local orchestration/tooling/tests/docs only; no live Worker, route, DNS, WordPress/Kinsta, R2 upload, dashboard production, analytics admin, source content, or locked PIB file was mutated.

09/02/2026 Resi Edge desktop native visual release gate addendum: Mark approved wiring the Balmoral blank-hero lesson into release control. `/Users/mark/Property_Analytics/scripts/validate_resi_edge_desktop_native_visual_gate.mjs` now renders the current shared runtime against a mocked native desktop page containing DAM logo, hero, `srcset`, and background-image URLs; verifies `x-vtr-desktop-mode:native-passthrough`; requires `0` direct `https://dam.getresi.co/` URLs in rendered desktop HTML/DOM; exercises `/__resi-edge/native-dam-asset?src=...` and requires `x-vtr-native-asset-repair: dam-proxy`; and uses Playwright to prove native header/hero/media paint without mobile-shell leakage or proxy request failures. `/Users/mark/Property_Analytics/scripts/resi_edge_deploy_adapter.py --apply` now blocks before route cleanup or Worker deploy unless the named property's latest gate artifact is passing and hash-matched to the current manifest, shared runtime, thin property Worker, and canonical Worker. Fresh local gate artifacts passed for all `27` live property manifests under `/Users/mark/Property_Analytics/reports/resi_edge_performance/desktop-native-visual-gate/`; pilot and empty legacy/reference stubs were excluded. Boundary: local proof tooling and deploy-adapter enforcement only; no live Worker, route, DNS, WordPress/Kinsta, dashboard production, R2 media/promo/freshness record, analytics admin, source content, or locked PIB file was mutated.

09/01/2026 Resi Edge emergency desktop native DAM asset proxy addendum: Mark reported that live desktop hero images were not showing after the desktop consent rollout; Balmoral Village (`GA4BV`, `balmoralvillageapts.com`) screenshot and browser console showed the native desktop page trying to load `https://dam.getresi.co/...` hero/media assets and the visitor browser failing DNS with `ERR_NAME_NOT_RESOLVED`. Mobile shell hero images were healthy because mobile uses same-origin R2 paths. The root cause was not a desktop topper or mobile shell issue: desktop remained `native-passthrough`, but the native Resi desktop page still depended on browser-resolved DAM URLs. `/Users/mark/Property_Analytics/ops/cloudflare/shared/resi-edge-package/runtime.mjs` now rewrites safe DAM image/SVG URLs in desktop/native-continuation HTML to `/__resi-edge/native-dam-asset?src=...`, and the property Worker fetches those assets server-side, caches them, strips cookies, preserves content type, and marks responses with `x-vtr-native-asset-repair: dam-proxy`. Balmoral was deployed first as Worker version `1dc05419-4f15-4354-95d6-136f208304d3`; screenshot proof `/Users/mark/Property_Analytics/reports/resi_edge_performance/native-dam-proxy-hotfix/20260901T212100Z/balmoral-live-proof/desktop.png` showed the native desktop hero restored. Fresh hash-tied local proofs passed for the remaining `26` live manifests with runtime SHA-256 `5e65e597a17a07da7d5fe1383fdb40a7dbce635ad02451ef98760b85e22ac310`, then each remaining property Worker was applied under per-property scope lock. Fleet live desktop proof passed for all `27/27` properties at `/Users/mark/Property_Analytics/reports/resi_edge_performance/native-dam-proxy-hotfix/20260901T212100Z/live-fleet-proof/native-dam-proxy-live-fleet-proof.json`: every desktop page stayed `native-passthrough`, rendered HTML had `0` direct DAM URLs, same-origin proxy URLs were present, and there were `0` DAM/proxy failed or bad responses. Boundary: native desktop layout/content was preserved; no desktop topper, mobile shell behavior, tours, specials, source content, WordPress/Kinsta, DNS, central service Worker, analytics admin, dashboard production, R2 hero/promo/media records, or locked PIB files were changed.

09/01/2026 Resi Edge desktop consent pill compact fleet refresh addendum: Mark approved a small desktop-only consent widget size adjustment after reviewing a legacy compact-pill reference and the Anatole canary. `/Users/mark/Property_Analytics/ops/cloudflare/shared/resi-edge-package/runtime.mjs` now injects a desktop-width size override for the existing consent pill on native passthrough pages only; the existing consent widget contract, cookie icon, `Preferences`, `Accept`, consent storage, Zaraz behavior, and mobile compact shell mode are unchanged. Anatole was deployed and visually approved first, with live proof screenshot `/Users/mark/Property_Analytics/reports/resi_edge_performance/desktop-consent-compact/20260901T204100Z/anatole-live-desktop-smaller.png` and Worker version `c503d12d-87e6-4090-9681-10344107a721`. Fresh hash-tied local central proofs then passed for the remaining `26` live manifests with runtime SHA-256 `382ca92ec08024f061aae209072f9c3ad096d3bcfc0832e9bf379b2d3eb321a8`, and each remaining property Worker was re-applied through the governed centralized deploy adapter under its own explicit scope lock. Apply summary is `/Users/mark/Property_Analytics/reports/resi_edge_performance/desktop-consent-compact/20260901T204100Z/remaining-applies/remaining-apply-summary.json`. Fleet live compact proof passed for all `27/27` properties at `/Users/mark/Property_Analytics/reports/resi_edge_performance/desktop-consent-compact/20260901T204100Z/live-compact-proof/live-compact-proof-summary.json`: every desktop pill measured `474x54`, every desktop path remained `native-passthrough`, and every mobile path retained `central_render_only` / `render-only` / `traffic-owner` topology. Boundary: no mobile layout/behavior, tours, specials, source content, WordPress/Kinsta, DNS, central service Worker, analytics admin, dashboard production, R2 hero/promo/media records, or locked PIB files were mutated.

09/01/2026 Resi Edge hero monitor same-URL hash fix and finish-out addendum: Mark approved fixing the same-URL hero freshness blind spot and finishing the remaining hero refresh targets. The root cause was two-part: The Vine's DAM conversion URL served original JPEG bytes to the generated-asset path but negotiated WebP bytes to monitor/acceptance source probes, and manifest-baseline monitor records could replace their source hash observation rather than preserving the baseline hash for same-URL comparison. `/Users/mark/Property_Analytics/apps/api/src/platform/resi-edge/hero-freshness-sync.ts` now preserves manifest-baseline hashes across runs and hashes original image responses with `Accept: image/jpeg,image/png,image/*,*/*;q=0.8`; `/Users/mark/Property_Analytics/scripts/collect_resi_edge_hero_freshness.py` uses the same source hash path, which aligns `/Users/mark/Property_Analytics/scripts/accept_resi_edge_generated_hero_assets.py` with generated packets. Tests added in `/Users/mark/Property_Analytics/apps/api/test/platform/resi-edge-hero-freshness-sync.test.ts` cover same-URL drift and negotiated WebP avoidance. `pop-brief-api` was deployed through the Keeper-backed Wrangler helper with `--minify` as version `a0e3ab01-f6a8-4d63-8873-79351e2eedac`. Production pre-acceptance proof at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-monitor-same-url-fix/20260901T201400Z/manual-sync-proof/20260901T201330Z/` correctly showed `26/27 current`, `1 refresh_needed`, and The Vine as the refresh-needed row. Finish-out then passed for The Vine, Townestone, Tuscany, Ventana, and Village Walk. Final readback `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-finishout/20260901T201600Z/final-readback/finishout-closeout-readback.json` proves each of the five targets has `freshness:current`, `media_state:accepted`, and `baseline:media_state`; final manual monitor run `20260901T201701Z` has matching latest/archive R2 receipts with `property_count:27`, `current_count:27`, `refresh_needed_count:0`, `source_missing_count:0`, and `source_error_count:0`. Boundary: shared API monitor/local collector/acceptance helper plus approved hero media assets/config/freshness records only; no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, locked PIB file, or Queue/Images consumer mutation occurred.

09/01/2026 Resi Edge concurrent one-step hero refresh batch addendum: Mark approved the remaining active production hero refresh group through the proven generated-asset lane using bounded concurrency `3`, per-property `run_resi_edge_hero_refresh.py --apply --skip-monitor-sync`, and one final manual hero freshness sync. Batch evidence lives at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-one-step-batch/20260901T194708Z/`; closeout readback is `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-one-step-batch/20260901T194708Z/final-readback/final-readback-summary.json`. Seventeen launched properties passed: Anatole, Axial, Calais, Champions, Forest View, Links, Luma, Park, Retreat, San Palmilla, Stonecreek, District, Harrison, Metropolitan, Phoenix, Whitney, and Timberlane. The Vine failed closed during acceptance because its DAM URL stayed the same while the source bytes changed between generation and acceptance; the generated packet SHA-256 was `6f8f3d429bced0680b79ba6e9cd88f271d4f0d66a906dd563f8c819ec7beaefd`, while later same-URL native readback was `02afb11370edd322593fce187b53a5e2e7c838cbb3bddb036da3e141f638e523`. Vine stable assets and config were uploaded, but accepted media-state records were not written by this run. The batch stopped and did not launch Townestone, Tuscany, Ventana, or Village Walk. Final manual monitor sync `20260901T195120Z` wrote matching latest/archive summaries with `property_count:27`, `current_count:27`, and `refresh_needed_count:0`; however, this exposed a monitor blind spot because the dashboard-facing monitor reported current while acceptance caught same-URL byte drift and Vine lacked media-state acceptance. System implication: the monitor should be hardened to track same-URL source hashes against accepted media-state or last-observed hashes before it is treated as complete media-state acceptance proof.

09/01/2026 Resi Edge manual hero freshness sync and Boulevard refresh addendum: Mark rejected waiting for a scheduled cron after a named hero refresh, so the hero freshness monitor now has an operator-triggered production path in addition to the 15-minute safety-net schedule. `pop-brief-api` exposes protected `POST /v1/platform/resi-edge/hero-freshness/sync`, using the same `/Users/mark/Property_Analytics/apps/api/src/platform/resi-edge/hero-freshness-sync.ts` routine as cron; deployed API Worker version is `cebebdbc-eb6d-466c-9ed4-165f6ca36634`. `/Users/mark/Property_Analytics/scripts/run_resi_edge_hero_freshness_sync_now.py` resolves platform Access service-token auth through Keeper/KSM, calls that endpoint, prints sanitized status/counts, and writes evidence under `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-freshness-manual-sync/<run-id>/`. `/Users/mark/Property_Analytics/scripts/run_resi_edge_hero_refresh.py --apply` now runs the manual monitor kick after the named property's generated assets, central config record, and media-state/freshness acceptance pass; `--skip-monitor-sync` is available only when an operator intentionally leaves summary advancement to cron. Boulevard at Lakeside (`OK4BL`, `blvdatlakeside.com`) passed the one-step hero refresh at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-one-step/blvdatlakeside-com/20260901T184925Z/`, using source `https://dam.getresi.co/19661/conversions/Home-Hero-full.jpg` with SHA-256 `38968e30c82f3655b869b3a544ca978dcf8a1d4e8704bfd286b9583bd70283b0`; `7/7` planned R2 objects uploaded, central config was refreshed, and media-state/freshness acceptance readback passed. Same-origin Boulevard hero proof matched AVIF `75,374` bytes SHA-256 `b4cf668562555f8a97698d18ee8bf5b275aa642ad096447006aefd95db628527` and WebP `78,874` bytes SHA-256 `e0eb8e1036dc9a0356ed1b01b14fb0af188b0b61ab5b2d8ba1ed0fa2f8b65249`. Manual production sync run `20260901T190413Z` completed immediately with `property_count:27`, `current_count:27`, `refresh_needed_count:0`, `source_missing_count:0`, and `source_error_count:0`; R2 readback proved `_latest-summary.json` and `_runs/20260901T190413Z.json` match, and Boulevard reads `current` / `recommended_action:none` / `baseline.system:media_state`. Boundary: shared API Worker route/tooling/docs plus Boulevard hero media/config/freshness proof only; no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, locked PIB file, Cloudflare Queue/Images consumer, or unrelated hero asset was mutated.

09/01/2026 Resi Edge hero freshness monitor repair and Balmoral refresh addendum: Mark asked to fix the monitor and proceed only to Balmoral Village (`GA4BV`, `balmoralvillageapts.com`). The monitor issue had two parts. First, production `pop-brief-api` was stale relative to local media-state-aware code and could overwrite accepted media-state properties as `refresh_needed`; the current API Worker was deployed as version `16d1080a-c0fd-4b73-9697-65756a77d043`, after which the 09/01/2026 18:15 UTC scheduled run wrote Axial and Balmoral individual records as `current` with `baseline.system:"media_state"`. Second, that same 18:15 UTC run archived a correct `27/27 current` summary but did not advance `_latest-summary.json`; `/Users/mark/Property_Analytics/apps/api/src/platform/resi-edge/hero-freshness-sync.ts` now uses bounded concurrent property scans, concurrent per-property record writes, and writes `_latest-summary.json` before the archived run receipt. Final tightened API Worker version: `c7ffe898-ad51-4068-a39d-a094fd49c7d2`. Production proof after the 09/01/2026 18:30 UTC cron showed `resi-edge-hero-freshness/_latest-summary.json` advanced to run `20260901T183031Z` with `property_count:27`, `current_count:27`, `refresh_needed_count:0`, `source_missing_count:0`, `source_error_count:0`, and queue production disabled. Final-version proof after the 09/01/2026 18:45 UTC cron on API Worker `c7ffe898-ad51-4068-a39d-a094fd49c7d2` advanced `_latest-summary.json` to run `20260901T184531Z` with the same `27/27 current`, `0` refresh needed result. Balmoral was refreshed through `/Users/mark/Property_Analytics/scripts/run_resi_edge_hero_refresh.py --property-code GA4BV --domain balmoralvillageapts.com --apply`; evidence lives at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-one-step/balmoralvillageapts-com/20260901T180616Z/`. Source truth is `https://dam.getresi.co/19636/conversions/Home-Hero-full.jpg`, SHA-256 `ebdb690b541d993249769b211fe78d678075684faf79cab47f54cf40a86d3e8c`; Balmoral same-origin hero readback matched AVIF `73,870` bytes SHA-256 `903ab60d7d06c1bff8557378dd26d835850864cb20bb1970aa5412914232d97b` and WebP `76,848` bytes SHA-256 `c602a756d39f90f6ad6aa935f1f632d45dd42b2a15ee97e9943e61e77f68633f`. Deployment note: `pop-brief-api` should be deployed through the Keeper-backed Wrangler helper with `wrangler deploy --minify` until the API bundle/deploy path is reduced; repeated non-minified uploads failed with Cloudflare upload socket closes. Boundary: no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, locked PIB file, Cloudflare Queue/Images consumer, or other property refresh was mutated.

09/01/2026 one-step Resi Edge hero refresh orchestrator addendum: Mark approved making the proven hero media refresh process more efficient one step at a time. Added `/Users/mark/Property_Analytics/scripts/run_resi_edge_hero_refresh.py` as the normal operator entry point for a named production hero media refresh. The wrapper requires exact `--property-code` and `--domain`, resolves one matching active manifest, writes evidence under `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-one-step/<domain>/<run-id>/`, and records command outputs, upload preview, acceptance/readback summary, status, and excluded mutation boundaries. Dry-run is default: it generates local assets, previews the upload plan, builds the config record locally, and builds local acceptance records only after verifying the live native hero source still matches the manifest/source hash. `--apply` runs the proven generated-asset sequence end to end: `/Users/mark/Property_Analytics/scripts/generate_resi_edge_assets.py`, `/Users/mark/Property_Analytics/scripts/upload_resi_edge_assets_to_r2.py --apply`, `/Users/mark/Property_Analytics/scripts/build_resi_edge_topper_config_records.py --upload`, and `/Users/mark/Property_Analytics/scripts/accept_resi_edge_generated_hero_assets.py --upload --readback`. Apply fails closed if upload, config record build/upload, acceptance upload, R2 readback, or same-origin asset hash readback fails. Focused tests live at `/Users/mark/Property_Analytics/tests/test_resi_edge_hero_refresh_orchestrator.py`. Boundary: this was local orchestration/tooling/documentation only; no property refresh, R2 mutation, Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, other property, or locked PIB file was mutated by this build.

09/01/2026 Axial generated-asset hero media refresh addendum: Mark stopped the failed queue path and directed returning to the proven method. Axial Buckhead (`GA4AB`, `axialbuckhead.com`) was refreshed through the generated-asset lane only: `/Users/mark/Property_Analytics/scripts/generate_resi_edge_assets.py`, `/Users/mark/Property_Analytics/scripts/upload_resi_edge_assets_to_r2.py --apply`, `/Users/mark/Property_Analytics/scripts/build_resi_edge_topper_config_records.py --upload`, and new `/Users/mark/Property_Analytics/scripts/accept_resi_edge_generated_hero_assets.py --upload`. The generated packet is `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-batch/20260901T162234Z/axialbuckhead-com/generated-assets.json`; source truth is `https://dam.getresi.co/18557/conversions/Home-Hero-full.jpg`, SHA-256 `6490f2271f444358e981c0c5d4aec796089a20324464f6ecb0c1fdba546cfc05`. R2 readback now shows media-state `accepted` and freshness `current` / `recommended_action:none`; live same-origin asset readback matches AVIF `76,035` bytes SHA-256 `03c95603be913c3e177f843ff3b9d74d6df346d1b0c3159d26d922fd416b8ba0` and WebP `77,936` bytes SHA-256 `767400ba00e30ee43b371b3dd8349373f51f1b89dc83572e98c44505a334a89c`. The operational correction is now explicit: production hero media refreshes default to generated assets plus acceptance records, while the Cloudflare Queue/Images consumer remains parked for focused diagnostics until Queue delivery and Worker mode activation are proven. Boundary: no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, other property, or locked PIB file was mutated.

09/01/2026 Axial hero media refresh canary stopped addendum: Mark approved trying the improved Cloudflare hero media refresh canary on Axial Buckhead (`GA4AB`, `axialbuckhead.com`). The dry-run packet passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-media-refresh-worker/axialbuckhead-com/20260901T160422Z/summary.json`, resolving the governed manifest and source image `https://dam.getresi.co/18557/conversions/Home-Hero-full.jpg` with SHA-256 `6490f2271f444358e981c0c5d4aec796089a20324464f6ecb0c1fdba546cfc05`. The live canary did not complete: the first attempt stopped before deploy/send on Cloudflare Queues code `11009` because idempotent create returned `Queue name ... is already taken`; the runner now accepts that wording. The second attempt produced a `skipped` receipt because the Worker consumed while effectively `disabled`; the runner now uses evidence-local `wrangler.disabled.toml` and `wrangler.canary.toml` files with absolute `main` and explicit vars instead of command-line var overrides. The final generated-config retry timed out without a run receipt. Final disabled deploy succeeded as Worker version `49dfd44d-7f83-44d5-b44e-34f048423947`, and the hero media queue/DLQ were purged. Post-stop readback showed no accepted media-state at `resi-edge-media-state/ga4ab-axialbuckhead-com/current.json`; the current Axial freshness record remains `refresh_needed` / `regenerate_hero_assets`, and latest candidate assets were absent. Boundary: no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, or locked PIB file was mutated. Axial must not be marked refreshed through this lane.

09/01/2026 Resi Edge hero media refresh canary process hardening addendum: Mark approved improving the Anatole Cloudflare hero media refresh canary process. The implementation tightened the shared consumer Worker and operator tooling only; no live Worker deploy, Queue send, property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, or locked PIB file was mutated. `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-hero-media-refresh-worker/worker.mjs` now stages candidate AVIF/WebP outputs under `resi-edge-media-refresh/_candidates/`, proves candidate readback, promotes only then to stable hero asset keys, proves stable readback, and writes deterministic `run_id`-based receipts when the queue message supplies a run id. Invalid messages and deterministic image budget failures now write non-retryable structured receipts and acknowledge rather than cycling through Cloudflare Queue retries; unknown/transient failures remain retryable. Added `/Users/mark/Property_Analytics/scripts/run_resi_edge_hero_media_refresh_canary.py` as the governed dry-run-first path for future named canaries. Default mode resolves property identity, validates the matching manifest, fetches and hashes the hero source, and writes an exact queue message preview; explicit `--apply` performs queue create/keep, disabled deploy, named canary deploy, queue/DLQ purge, one HTTP Queue message, deterministic R2 receipt polling, media-state/freshness/same-origin readback, and final disabled deploy through Keeper-backed Cloudflare auth. A non-mutating Anatole dry run passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-media-refresh-worker/anatoleatnorman-com/20260901T155825Z/summary.json` with source SHA-256 `9546cce3f519daf283bdf166ed5702087e1137164a82c906f1eb25b256067edb`.

09/01/2026 MKT-14465 The Vine FAQ edits applied addendum: Mark approved applying Christian Buckley's MKT-14465 FAQ edits for The Vine Kyle Parkway (`TX4EK`, `thevinekyle.com`). The change scope was 10 existing FAQ answer updates only, preserving existing Resi question text. The Resi Content Bridge applied 10 guarded V2 `PATCH /faqs/{id}` updates with originating system `MKT-14465`, writing `pond_content_change_requests` ledger rows and passing immediate V2 readback as `applied_readback_verified` for each item. Change request ids are `resi_content_bridge_30d4ed57c284b07e`, `resi_content_bridge_d4886a8539afaeb3`, `resi_content_bridge_1676901233680d15`, `resi_content_bridge_e4ed14658709d082`, `resi_content_bridge_0a8cff6aaa7627b3`, `resi_content_bridge_5e0585162d60c2a1`, `resi_content_bridge_aac0daa1156758f4`, `resi_content_bridge_ac90648abd465831`, `resi_content_bridge_a4d72df8d2aaa091`, and `resi_content_bridge_f5b2feba0a50e162`. A legacy V1 cache-clear request returned `202 Accepted` before Mark clarified that V1 is not the cache path for V2 sites; the corrected operating posture is Mark/manual Resi CP system-cache clear until Resi exposes V2 cache control. Post-update inventory run `resi_content_ae67caa8a04c` captured `52,429` content objects, `140,566` field facts, and `26` The Vine FAQs with the updated answers present. Live `https://thevinekyle.com/faqs/` proof returned HTTP `200`, Cloudflare cache status `EXPIRED`, and matched all 10 expected snippets; stale snippets `REMOVE SPECIAL`, `Hard Hat Tours`, `Move in by 10/31`, `$750 OFF`, `604 to 1,126`, `312-unit`, `San Antonio`, `breezeway garages`, `SAPD`, and `Bexar County Sheriff` were absent. Boundary: no host/admin, WordPress/Kinsta, Cloudflare, DNS, Resi Edge Worker, D1/KV, media, pricing/fee, or locked PIB mutation was performed.

09/01/2026 Resi V2-site cache-control correction: Mark clarified during The Vine FAQ edit testing that Resi V1 is a different/original site set, including examples such as Delta, Camber, and Cendana. V1 must not be treated as the cache-control path for Resi V2 sites such as The Vine. Resi does not have V2 cache control yet; for V2-site FAQ/content edits the correct proof path is V2 management readback, refreshed Data Pond inventory, Mark/manual Resi control-panel system-cache clear when needed, and rendered public website/browser proof.

09/01/2026 Anatole Cloudflare hero media refresh Worker canary addendum: Mark named Anatole at Norman (`OK4AN`, `anatoleatnorman.com`) as the first live Cloudflare hero media refresh queue/consumer canary. The canary scope was Anatole hero media infrastructure and records only; no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, or locked PIB file was mutated. The Cloudflare Queues `resi-edge-hero-media-refresh` (`436c730e72e94c75a41d5ccf084dcca1`) and `resi-edge-hero-media-refresh-dlq` (`b047e3db35d045948bff8799b371cfae`) were created through the Keeper-backed Wrangler helper path. The consumer Worker was deployed disabled first, then deployed in Anatole-only canary mode. The first live attempt surfaced a real Cloudflare Images encoder variance: WebP output for Anatole exceeded the strict `80,000` byte budget at the old floor. The Worker wrote no partial assets because both formats must transform before storage. The Worker and API queue producer policy now keep the strict budget while allowing WebP quality search to continue lower when needed. Fixed Worker version `622308d0-e6d5-4a6c-8e9e-b186ea7de853` processed one clean Anatole queue message with queue invocation `ok` and no exceptions. R2 run receipt: `resi-edge-media-refresh/_runs/20260901T011706Z-ok4an-anatoleatnorman-com-refreshed-9546cce3f519.json`. Media-state is accepted at `resi-edge-media-state/ok4an-anatoleatnorman-com/current.json`; freshness is `current` with `recommended_action:none` and baseline `media_state`. Same-origin readback matched the media-state hashes: AVIF `79,600` bytes SHA-256 `c70a8657be2abeec1eef910ee367c2dc12c6a96eb16a9d51b04cd5627a6ba06c`; WebP `77,938` bytes SHA-256 `5a493a1a7993231ee9411c1816a03a17225599878412746ca032ed122f9916c6`. The consumer was returned to disabled mode as version `c7e74811-bfbc-4d1e-a38d-9a21f23c6b0a`, and API queue production remains disabled. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-media-refresh-worker/anatoleatnorman-com/20260901T011706Z/anatole-hero-media-refresh-worker-canary-closeout.json`.

09/01/2026 Cloudflare-first Resi Edge hero media refresh lane addendum: Mark approved proceeding with the best-practice Cloudflare-first path for future hero media refreshes. The work prepared local capability and governance only; no Cloudflare queue was created, no Worker was deployed, and no property Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, source content, or locked PIB file was mutated. The API Worker hero freshness routine now compares native hero source facts against accepted `resi-edge-media-state/<property-code>-<domain>/current.json` records first, then falls back to the manifest when no accepted media-state exists. Queue production is present but disabled by default through `RESI_EDGE_HERO_MEDIA_QUEUE_ENABLED=false`, and it only emits new `refresh_needed` transitions to avoid repeated duplicate work on every `15` minute Cron wake. A standalone Worker at `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-hero-media-refresh-worker/worker.mjs` consumes `resi_edge_hero_media_refresh_queue.v1`, verifies the queued native source SHA before generating assets, uses Cloudflare Images to produce stable `750x1000` AVIF/WebP hero assets under the `80KB` mobile budget, writes R2-backed media-state and hero-freshness records, and stores run receipts under `resi-edge-media-refresh/_runs/`. The Worker is safe by default: `disabled` skips, `canary` processes only an allowlisted property code/domain, and `auto` is reserved for later explicit approval. Central contract, static validation, shared Cloudflare standards, and operator runbooks now document this lane.

09/01/2026 Resi Edge hero media refresh batch addendum: Mark confirmed the refreshed central flow and media handling is a better flow with stronger performance, so the preferred operating model is frequent read-only freshness detection followed by explicit governed image regeneration/storage only when approved. Mark approved generating and properly storing the hero images needed by the all-site media freshness check. The batch was limited to the 14 `refresh_needed` hero-media targets: Axial Buckhead (`GA4AB`, `axialbuckhead.com`), Balmoral Village (`GA4BV`, `balmoralvillageapts.com`), Creekside (`OK4CS`, `creeksideapt.com`), Links at Windsor Parke (`FL4WP`, `linksatwindsorparke.com`), Forest View (`TX4FV`, `liveatforestviewapts.com`), Canton Mill Lofts (`GA4CM`, `livecantonmill.com`), Luma Headwaters (`FL4LH`, `lumaheadwaters.com`), The Phoenix (`TX4PX`, `phoenixfortworth.com`), Stonecreek Ranch (`TX4ST`, `stonecreekranchapartments.com`), The Metropolitan (`KY4MP`, `themetropolitankentuckyapts.com`), The Vine Kyle Parkway (`TX4EK`, `thevinekyle.com`), Timberlane Village (`MO4TL`, `timberlanevillageapts.com`), Tuscany at Lindbergh (`GA4TU`, `tuscanylindbergh.com`), and Village Walk (`FL4VW`, `villagewalkapts.com`). The governed refresh path updated 13 manifest hero source pointers to current DAM conversion sources; The Vine was regenerated without another manifest mutation because its source pointer was already current. The asset generator produced fresh mobile hero AVIF/WebP derivatives and uploaded only the two stable hero R2 keys per target, for `28` planned uploads, `28` uploaded, and `0` failures. Live same-origin readback matched generated SHA-256 for `28/28` hero assets. Refreshed central config records were uploaded for all 14 targets, and the 14 matching hero-freshness records were uploaded. The final all-site hero freshness check reported `27/27` current, `0` refresh needed, `0` source missing, and `0` source errors. The active media-check cadence is every `15` minutes via the Cloudflare API Worker cron `*/15 * * * *` when `RESI_EDGE_HERO_FRESHNESS_SYNC_ENABLED=true`; this scheduled job writes R2 freshness records/summary receipts only and does not regenerate images or mutate Workers. Static validation and local central proof were refreshed and passed for all 14 so future deploy-adapter applies are not blocked by stale manifest hashes. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-refresh-batch/20260901T0020Z/hero-refresh-batch-closeout.json`. Boundary: no Worker, route, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 remaining-active-queue central render-only rollout addendum: Mark approved completing the remaining active Resi Edge central-renderer queue after Stonecreek Ranch. The resolved targets were The Metropolitan (`KY4MP`, `themetropolitankentuckyapts.com`), The Phoenix (`TX4PX`, `phoenixfortworth.com`), The Whitney (`GA4TW`, `thewhitneysandysprings.com`), Timberlane Village (`MO4TL`, `timberlanevillageapts.com`), Tuscany at Lindbergh (`GA4TU`, `tuscanylindbergh.com`), and Village Walk (`FL4VW`, `villagewalkapts.com`). Static validation and fresh hash-tied local central proof passed for all six, proving render-only central service, traffic-owner property Worker, bundled-vs-central shell equivalence, `10` drawer links, Tour enabled, no raw/unstyled state, and no broken images. The target config records were uploaded to remote R2, and only the six named centralized property Workers were re-applied under per-target scope locks. New Worker versions: The Metropolitan `ed9731b2-a922-4665-8df8-661e15fa735f`; The Phoenix `0024bb10-fdc9-4b8c-92a9-5e7edc252752`; The Whitney `24d81dea-38c8-4017-9c72-c5e8d8603b0e`; Timberlane Village `887f2488-2ddc-434e-b2e2-fe0262ff8b9c`; Tuscany at Lindbergh `0bf43c0b-37c2-417e-913d-6389f9201e1d`; Village Walk `127be4f0-e1d4-45a3-9ff6-8e75e26982bf`. Live browser/readback proof passed for all six: mobile `central_render_only`, central service `render-only`, property Worker `traffic-owner`, expected config key, desktop `native-passthrough`, styled/deduped native continuation, Tour links present, no old Heap id, no broken images, and no raw/unstyled signal. The Whitney correctly showed promo `edge_record_current`/`true`; the other five showed promo `edge_record_absent`/`false`. Phoenix and Timberlane each retained an immediate stale-mobile proof after deploy, then passed targeted recheck before rollout continued. `/__resi-edge/thin-health` passed and `/wp-login.php` stayed native/control-path with no Resi Edge headers. PSI was intentionally not rerun for this fast expansion batch. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-rollout/remaining-after-stonecreek/20260831T234256Z/remaining-central-rollout-closeout.json`. Boundary: no completed properties, shared central service deployment, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 next-five-after-Links central render-only rollout addendum: Mark approved five more active Resi Edge properties after the alpha-order run through Links at Windsor Parke. The resolved targets were Luma Headwaters (`FL4LH`, `lumaheadwaters.com`), Park on Wurzbach (`TX4WZ`, `parkonwurzbach.com`), Retreat at Kedron Village (`GA4KV`, `retreatatkedronvillage.com`), San Palmilla (`TX4SP`, `sanpalmilla-houston.com`), and Stonecreek Ranch (`TX4ST`, `stonecreekranchapartments.com`). Static validation and fresh hash-tied local central proof passed for all five, proving render-only central service, traffic-owner property Worker, bundled-vs-central shell equivalence, `10` drawer links, Tour enabled, no raw/unstyled state, and no broken images. The target config records were uploaded to remote R2, and only the five named centralized property Workers were re-applied under per-target scope locks. New Worker versions: Luma Headwaters `039d7ad9-4245-4491-bfaa-c124785e1bf6`; Park on Wurzbach `ed39cc98-b4e6-4e03-9444-25eca26ed339`; Retreat at Kedron Village `46983fa0-5cc6-4488-ba4c-943a0f8d471b`; San Palmilla `e086b704-1586-4ac8-8aeb-4c9dd3744cd5`; Stonecreek Ranch `37666584-4947-418b-9101-2e3f88d5e13b`. Live browser/readback proof passed for all five: mobile `central_render_only`, central service `render-only`, property Worker `traffic-owner`, expected config key, desktop `native-passthrough`, styled/deduped native continuation, Tour links present, no old Heap id, no broken images, and no raw/unstyled signal. Park on Wurzbach, San Palmilla, and Stonecreek Ranch correctly showed promo `edge_record_current`/`true`; Luma Headwaters and Retreat at Kedron Village showed promo `edge_record_absent`/`false`. `/__resi-edge/thin-health` passed and `/wp-login.php` stayed native/control-path with no Resi Edge headers. PSI was intentionally not rerun for this fast expansion batch. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-rollout/next-five-after-links/20260831T232627Z/next-five-central-rollout-closeout.json`. Boundary: no other property, shared central service deployment, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 next-five-after-Boulevard central render-only rollout addendum: Mark approved five more active Resi Edge properties after the alpha-order run through Boulevard at Lakeside. The resolved targets were Canton Mill Lofts (`GA4CM`, `livecantonmill.com`), Carlyle Place (`TX4CP`, `carlyleplacesa.com`), Creekside (`OK4CS`, `creeksideapt.com`), Forest View (`TX4FV`, `liveatforestviewapts.com`), and Links at Windsor Parke (`FL4WP`, `linksatwindsorparke.com`). Static validation and fresh hash-tied local central proof passed for all five, proving render-only central service, traffic-owner property Worker, bundled-vs-central shell equivalence, `10` drawer links, Tour enabled, no raw/unstyled state, and no broken images. The target config records were uploaded to remote R2, and only the five named centralized property Workers were re-applied under per-target scope locks. New Worker versions: Canton Mill Lofts `f978f3d3-3bad-4ce6-b04e-a43c33b2d007`; Carlyle Place `f8fca66b-9a84-4ca1-bb3a-7f21ce04d9d0`; Creekside `70522887-4383-4ecb-8a2d-33742382f3f6`; Forest View `7cab47e5-c98e-4975-bc7f-542c1c49597f`; Links at Windsor Parke `90bfabec-aed1-4400-8a7e-4129b35dc59d`. Live browser/readback proof passed for all five: mobile `central_render_only`, central service `render-only`, property Worker `traffic-owner`, expected config key, desktop `native-passthrough`, styled/deduped native continuation, Tour links present, no old Heap id, no broken images, and no raw/unstyled signal. Canton, Carlyle, and Links correctly showed promo `edge_record_current`/`true`; Creekside and Forest View showed promo `edge_record_absent`/`false`. `/__resi-edge/thin-health` passed and `/wp-login.php` stayed native/control-path with no Resi Edge headers. PSI was intentionally not rerun for this fast expansion batch. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-rollout/next-five-after-boulevard/20260831T231633Z/next-five-central-rollout-closeout.json`. Boundary: no other property, shared central service deployment, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 next-three-after-Anatole central render-only rollout addendum: Mark approved trying the next three active Resi Edge properties in alpha order after Anatole. The resolved targets were Axial Buckhead (`GA4AB`, `axialbuckhead.com`), Balmoral Village (`GA4BV`, `balmoralvillageapts.com`), and Boulevard at Lakeside (`OK4BL`, `blvdatlakeside.com`). Static validation and fresh hash-tied local central proof passed for all three, proving render-only central service, traffic-owner property Worker, bundled-vs-central shell equivalence, `10` drawer links, Tour enabled, no raw/unstyled state, and no broken images. The target config records were uploaded to remote R2, and only the three named centralized property Workers were re-applied under per-target scope locks. New Worker versions: Axial Buckhead `63e4925b-b940-475e-91fe-81d44fa9d698`; Balmoral Village `171a9b60-ea43-4bdb-98d4-943774a23c5e`; Boulevard at Lakeside `bcf76c83-c3d8-4148-b6d9-84ab28001e60`. Live browser/readback proof passed for all three: mobile `central_render_only`, central service `render-only`, property Worker `traffic-owner`, expected config key, desktop `native-passthrough`, styled/deduped native continuation, Tour links present, no old Heap id, no broken images, and no raw/unstyled signal. Balmoral correctly showed promo `edge_record_current`/`true`; Axial and Boulevard showed promo `edge_record_absent`/`false`. `/__resi-edge/thin-health` passed and `/wp-login.php` stayed native/control-path with no Resi Edge headers. PSI was intentionally not rerun for this fast expansion batch. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-rollout/next-three-after-anatole/20260831T230800Z/next-three-central-rollout-closeout.json`. Boundary: no other property, shared central service deployment, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 Anatole current central render-only refresh addendum: Mark approved refreshing Anatole at Norman (`OK4AN`, `anatoleatnorman.com`) through the current central render-only optimization setup. Fresh local proof passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-canary/anatoleatnorman-com/20260831T225030Z/local-proof/central-topper-local-proof.json`, proving render-only central service, traffic-owner property Worker, bundled-vs-central shell equivalence, `10` drawer links, Tour enabled, no broken images, and no raw/unstyled signal. Anatole's central config record `resi-edge-topper-config/ok4an-anatoleatnorman-com/current.json` was uploaded to remote R2, then only Anatole's centralized property Worker was re-applied on `anatoleatnorman.com/*`; `resi-edge-canonical-anatoleatnorman-com` is now Worker version `075a2d59-000a-4699-8eba-6f68ba68a3ee`. Live proof passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-canary/anatoleatnorman-com/20260831T225030Z/live-proof/central-topper-live-proof.json`: mobile `central_render_only`, central service `render-only`, property Worker `traffic-owner`, desktop `native-passthrough`, correct config key, absent promo state, Tour links present, styled/deduped native continuation, no old Heap id, no broken images, and no raw/unstyled page signal. PSI passed with mobile exact/fresh `100/100` and desktop exact/fresh `98/96`. Closeout evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-canary/anatoleatnorman-com/20260831T225030Z/anatole-central-refresh-closeout.json`. Boundary: no other property, shared central service deployment, DNS, WordPress/Kinsta, dashboard production, analytics admin, Ahrefs, source content, or locked PIB file was mutated.

08/31/2026 The Vine hero media refresh addendum: Mark approved refreshing only The Vine Kyle Parkway (`TX4EK`, `thevinekyle.com`) hero media after the seven-property media/special check found its native source changed from `https://dam.getresi.co/28429/Vine_Hero_06-19-26.jpg` to `https://dam.getresi.co/67405/conversions/Vine_Hero_08-31-26-full.jpg`. `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/thevinekyle-com.manifest.json` now points `mobile_shell.hero.source_image` at the `08/31/2026` source. The governed asset generator produced refreshed mobile hero derivatives and only the two stable hero R2 keys were uploaded: `resi-edge-assets/TX4EK/home/hero-mobile-750x1000.avif` and `resi-edge-assets/TX4EK/home/hero-mobile-750x1000.webp`. Vine's central config record and hero-freshness record were uploaded; post-refresh freshness is `current` with `recommended_action:none`, and live same-origin asset readback returned AVIF SHA-256 `1e4fd7bc2a9a8ff61a2612372c72503a1a02b258d9398578f7005e81341aa3c4` and WebP SHA-256 `62a1b7e333c1ff0fa44b2aa546a13584e3a8ea93af59bdbef55b94d2576d420b`. Live mobile shell still has the Vine special and Tour CTAs in the promo/header/drawer. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-freshness/thevinekyle-com/20260831T223736Z/hero-refresh-closeout.json`. Boundary: no other property, Worker, route, DNS, WordPress/Kinsta, forwarding, dashboard production, analytics admin, source content, or locked PIB file was mutated.

08/31/2026 Townestone lease-up Tour CTA visibility addendum: Mark approved a Townestone-only hotfix after confirming Townestone at 359 (`TX4FC`, `townestoneat359.com`) has no tours yet. The Vine Kyle Parkway (`TX4EK`, `thevinekyle.com`) was explicitly excluded and keeps tours live despite also being a lease-up. `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/townestoneat359-com.manifest.json` now sets `mobile_shell.navigation.tour_enabled:false` while preserving the stored Tour URL for future approved re-enable. `/Users/mark/Property_Analytics/ops/cloudflare/shared/resi-edge-package/runtime.mjs` hides header/drawer Tour CTAs only when the selected manifest explicitly disables tours; all other manifests default to Tour enabled. Local/live proof scripts now support no-tour targets by checking rendered DOM Tour links rather than generic analytics parser text. Evidence: local proof `/Users/mark/Property_Analytics/reports/resi_edge_performance/townestone-tour-hide-hotfix/20260831T221152Z/local-proof/central-topper-local-proof.json`; live proof `/Users/mark/Property_Analytics/reports/resi_edge_performance/townestone-tour-hide-hotfix/20260831T221301Z/live-proof/central-topper-live-proof.json`; central service Worker version `b826f99b-b9c6-489d-b8a8-0a16fdcc6f52`; Townestone property Worker version `b5223bfa-b304-4673-a972-1aeb5e12dfff`. Live mobile readback showed `central_render_only`, `render-only`, `traffic-owner`, drawer count `11`, `tour_link_count:0`, promo `edge_record_absent`/`false`; desktop stayed `native-passthrough`. Boundary: no Vine manifest/property Worker, dashboard production, WordPress/Kinsta, DNS, forwarding, source content, analytics admin, Ahrefs, or locked PIB file was mutated.

08/31/2026 first five pilots plus Vine/Townestone central fast rollout addendum: Mark clarified the target set as Champions Green (`GA4CG`, `championsgreen-ga.com`), Ventana (`TX4VE`, `ventanaapts.com`), The Harrison (`GA4TH`, `theharrisonsandysprings.com`), The District Universal Boulevard (`FL4DU`, `thedistrictuniversal.com`), Calais Midtown (`TX4MI`, `calaismidtownapartments.com`), The Vine Kyle Parkway (`TX4EK`, `thevinekyle.com`), and Townestone at 359 (`TX4FC`, `townestoneat359.com`). The central rollout lane was tightened after an initial no-mutation Champions stop exposed an Anatole-specific `10`-link proof assumption; local and live proof now require the selected manifest's full nav count, with a minimum `10`, and compute the expected config key from the named property/domain. The fast rollout passed all seven with per-property scope locks, static validation, local hash-tied central proof, R2 config record upload, centralized property Worker apply, mobile/desktop/control/health live readback, and lock clear. Summary evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-rollout/fast-first-five-plus-vine-townestone/20260831T215136Z/central-topper-fast-rollout-summary.json`. All seven live mobile readbacks now report `central_render_only`, `render-only`, and `traffic-owner`; desktop reports `native-passthrough`; WordPress login/control checks did not expose central topper headers. Representative full browser/continuation proof and PSI were run for Champions Green, The Vine, and Townestone, with mobile `100` and desktop fresh/current `97` or better in the representative samples. Boundary: no `pilot.venterradev.com`, dashboard production, WordPress/Kinsta, DNS, forwarding, provider-admin, Ahrefs, source content, other property, or locked PIB mutation occurred.

08/31/2026 Anatole central render-only live canary addendum: Mark approved a tight Anatole-only live test of the corrected render-only central topology. Fresh local proof passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-local-proof/ok4an-anatoleatnorman-com/20260831T213027Z/central-topper-local-proof.json`. The live canary uploaded only Anatole's remote R2 config record `resi-edge-topper-config/ok4an-anatoleatnorman-com/current.json`, deployed central service Worker `resi-edge-topper-service` version `017bb5be-34a9-41dd-9c4f-15fba1fa8f5e`, and deployed Anatole property Worker `resi-edge-canonical-anatoleatnorman-com` version `4c58e014-a69a-4654-ba84-c2f1cc519aa5` on route `anatoleatnorman.com/*` with `RESI_EDGE_TOPPER` and `RESI_EDGE_ASSETS` bindings. Immediate live proof passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-canary/anatoleatnorman-com/20260831T213027Z/live-proof-continuation/central-topper-live-proof.json`: mobile GET used `central_render_only`, `render-only`, `traffic-owner`, current runtime `2026-08-31.edge-promo-record-v1`, and the Anatole config record; desktop GET used `native-passthrough` with no mobile shell; `/wp-login.php` stayed native/control-path; mobile native continuation loaded, hid native header/hero/welcome/features, carried YooTheme/theme styles, and showed no raw/unstyled signal, old Heap `676880719`, horizontal overflow, or broken image issue. Boundary: Anatole canary remains live; expansion is not authorized. No DNS, WordPress/Kinsta, forwarding, dashboard production, GA4 Admin, Zaraz tool config, Heap config, Ahrefs, source content, other property, or locked PIB mutation occurred.

08/31/2026 Resi Edge central topper render-only local proof addendum: after the Anatole central canary failed visible-page proof, the topology was corrected locally so the central service is no longer a full traffic/origin handler. `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-topper-service/worker.js` now exposes health and `/__resi-edge/render/mobile-shell` only, and returns a closed render-only response for other paths. `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-thin-property-worker/worker.js` now owns target host checks, WordPress/admin/control-path bypass, R2 asset serving, same-origin native asset repair, Contentsquare suppression, `llms.txt`, native continuation, desktop native pass-through, and fallback behavior, while delegating only mobile homepage shell rendering to the `RESI_EDGE_TOPPER` service binding. New proof harness `/Users/mark/Property_Analytics/scripts/validate_resi_edge_central_topper_local.mjs` materializes local runtime/service/Worker modules, mocks R2 config/promo records and native origin HTML, compares bundled mobile-shell output to the central-rendered candidate, confirms central non-render traffic is blocked, and captures Playwright mobile/desktop screenshots. Anatole-only local proof passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-local-proof/ok4an-anatoleatnorman-com/20260831T212330Z/central-topper-local-proof.json`, with latest pointer `/Users/mark/Property_Analytics/reports/resi_edge_performance/central-topper-local-proof/ok4an-anatoleatnorman-com/latest-central-topper-local-proof.json`. `/Users/mark/Property_Analytics/scripts/resi_edge_deploy_adapter.py` now blocks `--topper-mode centralized --apply` unless that local proof artifact is passing and its manifest/runtime/central-service/property-Worker hashes match current source. Boundary: local proof and apply-guard work only; no live Worker, route, DNS, R2 upload, cache purge, WordPress/Kinsta, GA4 Admin, Zaraz tool, Heap config, Ahrefs, dashboard production, source content, other property, or locked PIB mutation occurred. Production default remains bundled until Mark explicitly approves another live central retry.

08/31/2026 Resi Edge central topper runtime addendum: Resi Edge has a central topper topology design, but the first Anatole-only central canary failed visible-page proof and was emergency-restored. The governing contract is `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/resi-edge-central-topper-runtime.v1.json`; the config-record builder is `/Users/mark/Property_Analytics/scripts/build_resi_edge_topper_config_records.py`; the central service Worker is `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-topper-service/worker.js`; the thin property Worker is `/Users/mark/Property_Analytics/ops/cloudflare/resi-edge-thin-property-worker/worker.js`. `/Users/mark/Property_Analytics/scripts/resi_edge_deploy_adapter.py` supports explicit `--topper-mode bundled|centralized`, with `bundled` remaining the fleet default. The central model treats promo and hero freshness as data records in R2, not runtime forks. Anatole at Norman (`OK4AN`, `anatoleatnorman.com`) received remote R2 config record `resi-edge-topper-config/ok4an-anatoleatnorman-com/current.json`, central service Worker version `96839b0a-4792-423b-aa19-b753d7853cc2`, and thin property Worker version `58534ee4-6838-44f6-bd0e-8079e155b72f`. Header/contract proof initially passed, but the visible page broke by exposing broken native-continuation/native rendering. Emergency restore removed the Anatole Worker route `anatoleatnorman.com/*`, purged Anatole's Cloudflare cache, and visually verified native Kinsta/Resi desktop and mobile rendering. Boundary: central expansion is paused; do not retry or expand central/thin delivery without an offline visual-proof fix and explicit Mark approval. No WordPress/Kinsta mutation, analytics admin mutation, Ahrefs mutation, source content mutation, dashboard production mutation, or locked PIB mutation occurred.

08/31/2026 Resi Edge hero freshness monitor addendum: the Resi Edge launch/optimization system now has a read-only hero source freshness lane so upstream source-media changes cannot silently leave optimized edge assets stale. `/Users/mark/Property_Analytics/apps/api/src/platform/resi-edge/hero-freshness-sync.ts` is wired into the existing independent API Worker scheduler in `/Users/mark/Property_Analytics/apps/api/src/index.ts`, using the existing `RESI_EDGE_ASSETS` R2 bucket and `15` minute Cron cadence. It reads each active native desktop/cache-busted homepage, extracts the hero `data-src`, compares it to the manifest source and prior recorded source hash, and writes R2 receipts at `resi-edge-hero-freshness/<property-code>-<domain>/current.json`, `resi-edge-hero-freshness/_latest-summary.json`, and `_runs/<run-id>.json`. `/Users/mark/Property_Analytics/scripts/collect_resi_edge_hero_freshness.py` provides the local Data Pond/evidence fallback and feeds `/Users/mark/Property_Analytics/scripts/build_resi_edge_launch_dashboard_snapshot.py`; the protected launch dashboard now displays a compact Hero Media freshness indicator. First local 27-property packet: `/Users/mark/Property_Analytics/reports/resi_edge_performance/hero-freshness-sync/20260831T183924Z/`, with `27` checked, `13` current, `14` refresh needed, `0` source missing, and `0` source errors. API Worker version `9915cbeb-80fb-4778-914c-dbdc2fc59f74` and dashboard Pages deployment `https://2beb0c43.resi-edge-launch.pages.dev` are live; custom host returned `HTTP 200`. Boundary: this is detection/evidence/dashboard only and does not mutate property Workers, routes, DNS, forwarding, WordPress/Kinsta, analytics, R2 image assets, source content, optimization applies, or locked PIB files.

08/31/2026 Resi Edge promo freshness addendum: the Resi Edge package now has a canonical edge-record contract for mobile topper specials. The shared runtime reads per-property promo JSON from `RESI_EDGE_ASSETS` at `resi-edge-promo/<property-code>-<domain>/current.json`. The Cloudflare API Worker scheduled routine `/Users/mark/Property_Analytics/apps/api/src/platform/resi-edge/promo-record-sync.ts` refreshes active Resi Edge promo records every `15` minutes from the live ThirtyLines `propertyBannerSpecial` field and writes R2 receipts at `resi-edge-promo/_latest-summary.json` and `resi-edge-promo/_runs/<run-id>.json`; `/Users/mark/Property_Analytics/scripts/sync_resi_edge_promo_records.py` remains the local manual fallback using Data Pond snapshots. Manifest promo values are resilience fallback only and cannot satisfy final live proof; `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` now checks promo readout headers, stale state, manifest fallback, and rendered/header state agreement. The Cloudflare API Worker routine was deployed through Keeper-backed Wrangler as version `886f816c-a34e-4a56-ad88-c8afc88226d5`; the first proven remote scheduled receipt at `08/31/2026 11:30 AM CDT` wrote `27` active records, found `0` missing feed rows, and marked `14` active specials present. Evidence copy: `/Users/mark/Property_Analytics/reports/resi_edge_performance/promo-record-sync/cloudflare-api-worker-deploy-20260831T1630Z/latest-summary.json`. The API Worker scheduled shell now runs Captain, PIB, and Resi Edge promo jobs independently so an unrelated scheduled-lane error cannot suppress promo freshness. Boundary: Cloudflare API Worker scheduled routine deployment and R2 promo-record writes only; no property Worker route, DNS, WordPress/Kinsta, Zaraz, GA4, Heap, Ahrefs, cache, dashboard, or locked PIB behavior was mutated.

08/31/2026 Captain routine scheduler addendum: Captain support-agent execution is now governed by an explicit Cloudflare/D1 due queue instead of fixed Cron slot buckets. `/Users/mark/Property_Analytics/apps/api/src/platform/captain/runtime.ts` syncs active `captain_support_agents` into `captain_routine_schedule`, leases due rows, executes bounded batches through the existing Captain Runtime support-agent path, writes normal `captain_agent_runs`, and advances daily/weekly routines by completion state. The schedule contract is documented in `/Users/mark/Property_Analytics/config/captain_active_routine_manifest.json`: each active Captain property has `8` daily lanes and `3` weekly lanes. The API Worker Cron now wakes every `15` minutes but only runs due rows. Migration files are `/Users/mark/Property_Analytics/apps/api/migrations/0068_create_captain_routine_schedule.sql` and `/Users/mark/Property_Analytics/infra/migrations/045_create_captain_routine_schedule.sql`; operator runbook is `/Users/mark/Property_Analytics/docs/CAPTAIN_ROUTINE_SCHEDULER_RUNBOOK_2026-08-31.md`. Boundary: this mutates only Captain Runtime routine/schedule/watch/action state through existing D1 paths and does not mutate Jira, Confluence, Microsoft 365, intranet systems, source tickets, Resi content, Cloudflare zone settings, or locked PIB files.

08/28/2026 Resi Edge CTA study event-collection correction addendum: the launch CTA study audit found a collection/taxonomy asymmetry in the governed GA4 event import. The daily GA4 event collector requests only the official registry intent-event allowlist; the `03/25/2026` pilot properties were tagged `site_type: resi`, while the `08/19/2026` twenty-property Phase 2 launch cohort was not, so the Phase 2 import initially missed Resi-specific/current runtime event names. `/Users/mark/Property_Analytics/config/venterra_properties_official.json` now tags the exact twenty Phase 2 properties as `site_type: resi` and adds `schedule_tour_click`, `apply_now_click`, `phone_click`, `find_your_home_click`, and `find_home_click` to the Resi event allowlist. The affected Phase 2 window was backfilled for `08/19/2026` through `08/27/2026`, adding captured runtime events such as `find_your_home_click` `178`, `schedule_tour_click` `82`, and `apply_now_click` `70` in the local Data Pond. `/Users/mark/Property_Analytics/utils/adhoc_report_sources.py` now groups those names in the `resi_edge_launch_cta_study` taxonomy and frames `resi_3d_tour_view` as tracking-sensitive: direct GA4 source checks still show `0` Phase 2 events under that exact event name, but the report no longer presents that as proof of no 3D-tour behavior. Corrected validated packet: `/Users/mark/Property_Analytics/reports/adhoc_executive/resi_edge_launch_cta_study/20260828_160840_contained-normalized-resi-edge-launch-cta-study-comparing-the-03-25-2026-pilot-l/`. Boundary: Data Pond import configuration and governed ad hoc report taxonomy only; no locked PIB generation/rendering/sending files, live Resi Edge Worker, Cloudflare route, DNS, WordPress/Kinsta, Zaraz Admin, Ahrefs, R2, cache, or live site state was changed.

08/28/2026 Resi Edge launch CTA study addendum: the governed Ad Hoc Executive Report System now includes `resi_edge_launch_cta_study` for comparing first-nine-day CTA/event behavior between the 03/25/2026 five-property pilot cohort and the 08/19/2026 twenty-property Resi Edge launch cohort. The implementation lives in `/Users/mark/Property_Analytics/utils/adhoc_report_sources.py`, is exposed by `/Users/mark/Property_Analytics/scripts/run_adhoc_report.py`, renders through `/Users/mark/Property_Analytics/utils/outlook_report_builder.py`, and archives packets under `/Users/mark/Property_Analytics/reports/adhoc_executive/resi_edge_launch_cta_study/`. The builder resolves all properties through the governed property identity matrix, reads local GA4 daily metrics and GA4 event facts, normalizes legacy/SightMap/Resi Edge event names into comparable CTA categories, includes `form_submit` in high-intent outcomes, keeps `form_start` in the supporting behavior layer, adds PDF downloads plus SightMap unit exploration/search refinement/inventory exposure categories, and includes launch-day-indexed curves, cohort/pre-period scorecards, CTA mix, 3D Tour sensitivity, property detail, raw event names, and daily-row coverage gaps. After Mark rejected the first normalized email body as too wide in Outlook, the shared ad hoc Outlook renderer was corrected to use fixed-layout tables, hard word-wrapping, and KPI row widths that stay within the email body; the Resi Edge CTA Study HTML now keeps wide detail views in the workbook rather than the email body. Latest contained sent packet: `/Users/mark/Property_Analytics/reports/adhoc_executive/resi_edge_launch_cta_study/20260828_145921_contained-normalized-resi-edge-launch-cta-study-comparing-the-03-25-2026-pilot-l/`. Boundary: this is a report-engine extension only; no locked PIB generation/rendering/sending files, standalone renderer/sender, source-system mutation, Resi Edge Worker, Cloudflare route, DNS, WordPress/Kinsta, or live-site state was changed.

08/28/2026 Resi inventory provenance in AI Content Suite addendum: Site Content Creator now has a read-only source-provenance layer that connects its captured live sections and Specs structural slots to the existing governed Resi content inventory. The matcher at `/Users/mark/Property_Analytics/apps/api/src/platform/site-content/resi-section-mapping.ts` uses page type, expected Specs slot, captured headings/copy/bullets, Resi object type, Resi public copy, property impact count, and existing safe-field classification to return `property_matched`, `property_suggested`, `global_locked`, `not_resi_backed`, or `unavailable`. `/Users/mark/Property_Analytics/apps/api/src/routes/admin-site-content.ts` resolves the current property from the latest Resi V2 property snapshot using normalized name and source URL host, reads the existing inventory tables, and attaches provenance only to the composed response. It does not add heuristic bindings to the binding table or write to Resi. A global/multi-property Resi match is a server-enforced Site Content lock: the rewrite API recomputes the scope before accepting a save and returns a governance block if it is not property-safe. The AI Content Suite drawer displays the Resi source as a separate context surface, preserving the distinction between live source facts, Specs structure, Intelligence Office guidance, Captain memory, and editable working draft. Resi Content Bridge remains the sole approved live Resi apply/readback path.

08/28/2026 Resi Edge GSC indexing watch addendum: the first-20 Resi Edge launch now has a database-backed Google indexing watch surfaced inside PIB v2.3.1. `/Users/mark/Property_Analytics/scripts/collect_resi_edge_gsc_indexing.py` is a read-only collector/importer that resolves active Resi Edge launch properties from the launch dashboard snapshot and active manifests, inspects the standard migrated Resi page set for each vanity domain, and writes canonical rows into `/Users/mark/Property_Analytics/data/portfolio_analytics.db` table `gsc_url_inspection`. The 08/28/2026 launch packet `/Users/mark/Property_Analytics/reports/gsc_indexing/resi_edge_20/20260828T150258Z` was imported with evidence at `/Users/mark/Property_Analytics/reports/gsc_indexing/resi_edge_20/20260828T161641Z/summary.json`, with convenience pointer `/Users/mark/Property_Analytics/reports/gsc_indexing/resi_edge_20/latest-db-watch.json`: `220` inspected URLs, `20` properties, `220` successful URL Inspection payloads, `16/20` indexed roots, `6/20` complete standard-path sets indexed, and `0` blocked/fetch issue pages. `/Users/mark/Property_Analytics/Data_Collection/orchestration/daily_master_collection.py` now runs this Resi Edge watch after the broader GSC URL Inspection/action packet, using timeout-bound `AuthorizedSession` REST calls to avoid the prior hanging URL Inspection path. PIB v2.3.1 now builds `inspection.standard_watch` from canonical DB evidence and renders a `GSC Indexing Watch` panel in the Search Performance/GSC area with root page status, standard pages indexed, watched paths, and latest inspection date. Regression coverage at `/Users/mark/Property_Analytics/tests/test_pib_gsc_indexing_watch.py` prevents false positives such as counting `Crawled - currently not indexed` as indexed and confirms vanity-domain rows are preferred over legacy `venterraliving.com` rows. Boundary: this is read-only GSC reporting/database/PIB surfacing only; it does not submit URLs, request indexing, mutate sitemaps, edit content, change Cloudflare/Resi/WordPress/Kinsta, deploy Workers, or alter live domains.

08/27/2026 Agent Readiness weekly monitor addendum: added and deployed the dedicated Cloudflare Worker `/Users/mark/Property_Analytics/ops/cloudflare/agent-readiness-monitor/` for progressive weekly Agent Readiness scanning. The Worker is live at `https://agent-readiness.venterrawebops.com` with health, status, targets, and results endpoints, and current deployed version `78ba56d4-179e-4500-9d10-149887b26e3d`. D1 migration `/Users/mark/Property_Analytics/apps/api/migrations/0067_create_agent_readiness_tables.sql`, mirrored by `/Users/mark/Property_Analytics/infra/migrations/044_create_agent_readiness_tables.sql`, adds `agent_readiness_targets`, `agent_readiness_runs`, `agent_readiness_results`, and `agent_readiness_check_results` to `pop-brief-db`. The target seed utility `/Users/mark/Property_Analytics/scripts/seed_agent_readiness_targets.py` reads the governed property identity matrix and active Resi Edge manifests, resolving property identity through the canonical helper, and initial live seed applied `116` active targets: `94` corporate property pages and `22` Resi vanity domains. The Worker runs every six hours, processes due targets in batches of `8`, stores raw Cloudflare scanner evidence in R2 under `agent-readiness/raw/`, stores run manifests under `agent-readiness/runs/`, and advances each target by its seven-day cadence. Health/status smoke passed on 08/27/2026; the protected manual run endpoint fails closed until `AGENT_READINESS_ADMIN_SECRET` is represented in Keeper/KSM and set as a Worker secret. Runbook added at `/Users/mark/Property_Analytics/docs/AGENT_READINESS_MONITOR_RUNBOOK_2026-08-27.md`. Boundary: this is read-only scanner/reporting infrastructure only; no DNS, robots.txt, WordPress/Kinsta, Resi Edge Worker, content, Cloudflare zone configuration, Jira, Confluence, Microsoft 365, source-system, or locked PIB mutation is performed.

08/27/2026 Cloudflare AI Playground future-tool candidate addendum: `https://playground.ai.cloudflare.com/` has been cataloged as a watchlist tool for future Cloudflare-native AI exploration. Current live surfaces observed are Model mode for Workers AI prompt/model/MCP experiments, Compare mode for side-by-side answers with and without selected data sources, and Generative UI for prompt-generated component layouts. Voice agent is present but marked coming soon, and RAGBench diagnostics exists in the application bundle but is disabled in live config. Potential fit is low-friction prototyping before any Data Pond use of Workers AI, AI Gateway, MCP tools, or generated UI patterns. Boundary: this is catalog-only; no Cloudflare account state, Worker, Gateway, credential, Data Pond runtime, source system, Resi Edge package, or locked PIB file was changed. Any future production use must follow Keeper/KSM credential discipline and existing Cloudflare helper/deployment lanes.

08/27/2026 AI Content Suite global-block edit-lock addendum: the Pond-native content editor now distinguishes property-editable content from Venterra/global site blocks. `/Users/mark/Property_Analytics/apps/api/src/routes/admin-site-content.ts` decorates mapped sections with editability posture (`editable`, `locked_global`, `invalid_missing`, or `needs_mapping`) and rejects rewrite saves when a mapping is missing, unmapped, About Venterra, or detected as a Venterra/global program block such as Live Easy, smart-home/high-tech, resident experience, or corporate pet-policy content. `/Users/mark/Property_Analytics/apps/web/src/components/site-content-creator-page.tsx` now shows separate editable and locked/global counts, lock badges in the horizontal section rail, inspect-only chips on locked page-canvas blocks, and a read-only drawer with the reason and flags instead of rewrite controls. Deployed to API Worker version `d0c6efb8-bdcb-4152-b86e-35256580d32c` and Pond web production branch `https://15804750.property-analytics.pages.dev`; protected custom route `https://pond.venterrawebops.com/site-content` returned expected Cloudflare Access `302`. Boundary: this is classification and Pond UI/API guardrail work only; it does not delete captured content, publish Resi content, mutate the schema, call model providers, edit source systems, or touch locked PIB files.

08/28/2026 Resi inventory provenance addendum: AI Content Suite now composes a read-only Resi provenance result for each Specs-mapped, captured live section rather than treating a captured section as independently editable by default. `/Users/mark/Property_Analytics/apps/api/src/platform/site-content/resi-section-mapping.ts` evaluates page/slot affinity and content language against existing Resi inventory objects, and returns property matched, property suggested, global locked, not Resi backed, or unavailable. `/Users/mark/Property_Analytics/apps/api/src/routes/admin-site-content.ts` resolves the Data Pond property against the latest Resi snapshot by normalized name plus direct/nested source URL host, reads only property-linked safe fields, and recomputes status immediately before a rewrite save. Global/shared objects are locked by the API and return `GOVERNANCE_BLOCKED`; no automatic binding is stored. `/Users/mark/Property_Analytics/apps/web/src/components/site-content-creator-page.tsx` renders that source context separately from live copy, Specs contract, and drafting controls. Published API Worker version `85c3c98a-c893-48fe-a44a-952ae90b1064` and Pages deployment `https://7d3dece2.property-analytics.pages.dev`; Worker/API health returned HTTP `200`, while protected `https://app.venterradev.com/site-content` returned expected Access `302`. Boundary: source-provenance, UI, and save-guard work only; no Resi source content, binding table, D1 schema, or locked PIB behavior was mutated.

08/27/2026 Resi Edge scope lock and operator handoff addendum: after repeated agent drift during Resi Edge optimization, the workstream now has both a mechanical scope lock and a handoff package. `/Users/mark/Property_Analytics/docs/RESI_EDGE_OPTIMIZATION_OPERATOR_HANDOFF_PACKAGE_2026-08-27.md` is the explicit single-property operator packet for future Codex instances: it documents required first reads, authority rules, canonical package components, fixed invariants, scope-lock commands, target input template, static validation, `plan -> stage -> apply --require-live-proof`, dashboard finalization, PSI/analytics/control-path/mobile-shell rules, failure handling, and concise reporting templates. `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` now blocks `plan`, `stage`, and `apply` unless `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/active-resi-edge-scope-lock.json` is `ACTIVE`, unexpired, and names the exact property code/domain/mode approved by Mark in the current instruction; `/Users/mark/Property_Analytics/scripts/resi_edge_deploy_adapter.py --apply` enforces the same lock for direct deploy attempts. `/Users/mark/Property_Analytics/scripts/set_resi_edge_scope_lock.py` creates or clears that lock. Regression coverage is in `/Users/mark/Property_Analytics/tests/test_resi_edge_scope_lock.py`. Boundary: this is operator-control documentation and local runner/deploy-adapter guarding only; no live site, Cloudflare route, dashboard, DNS, WordPress/Kinsta, Zaraz, GA4, Heap, Ahrefs, R2, cache, source data, API schema, or locked PIB file was changed.

08/27/2026 AI Content Suite correction addendum: Mark clarified that VACS should not be a bridge or separate work surface and that Content Office is legacy/busted for the active content lane. The product direction is one in-Pond content workspace: AI Content Suite over live mapped page content. `/Users/mark/Property_Analytics/apps/web/src/app/vacs/page.tsx` now renders the same AI Content Suite editor as a compatibility route rather than launching or explaining a separate VACS destination. `/Users/mark/Property_Analytics/apps/web/src/components/site-content-creator-page.tsx` now frames the workflow as property -> page -> mapped live sections -> click-to-edit drawer, with a compact section rail, full-width live page/section canvas, visible click affordances, and drawer-based editing with AI working context. Do not reintroduce a permanent vertical editor column; the page/section map is the primary reading surface. `/Users/mark/Property_Analytics/apps/web/src/lib/permissions.ts`, `/Users/mark/Property_Analytics/apps/web/src/app/page.tsx`, and `/Users/mark/Property_Analytics/apps/web/src/app/dock/page.tsx` now promote AI Content Suite / `/site-content` as the active content editing destination while retaining old Content Office and VACS route concepts only as legacy/admin/compatibility paths. Corrected Cloudflare Pages production-branch deployment `https://c4f76a9d.property-analytics.pages.dev` returned HTTP `200` for `/site-content` and `/vacs`; the protected custom Pond route `https://pond.venterrawebops.com/site-content` returned expected Cloudflare Access `302`. Built/deployed bundle checks confirmed current `AI Content Suite`, `Mapped live page`, `Click to edit`, `AI working context`, and `VACS in Pond` markers while finding no old external-launch markers. Boundary: this is hosted Pond UI/navigation/product-composition work only; it does not write Resi content, mutate D1/R2, call source-system APIs, change Jira/Confluence/Microsoft 365, or touch locked PIB files.

08/27/2026 superseded VACS bridge note: the earlier same-day VACS bridge direction and deployment `https://495fa2af.property-analytics.pages.dev` are superseded by the AI Content Suite correction above. Do not treat VACS as a separate launcher/work surface for this lane, and do not promote old Content Office as the active content editing destination unless Mark explicitly reverses the current direction.

08/26/2026 Resi Edge optimization day closeout addendum: the first 20-property launch dashboard now reports `10/20` optimized proof complete after Forest View, Links at Windsor Parke, and Luma Headwaters joined the governed optimized set. Forest View (`TX4FV`, `liveatforestviewapts.com`) passed live apply at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/liveatforestviewapts-com/apply-20260827T000041Z/` with `57/57` gates and PSI mobile `98`, desktop `98`. Links at Windsor Parke (`FL4WP`, `linksatwindsorparke.com`) passed live apply at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/linksatwindsorparke-com/apply-20260827T003526Z/` with `57/57` gates, PSI mobile `100`, desktop `98`, and dashboard finalization to `https://0c575cc0.resi-edge-launch.pages.dev`. Luma Headwaters (`FL4LH`, `lumaheadwaters.com`) passed live apply at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/lumaheadwaters-com/apply-20260827T004426Z/` with `57/57` gates, PSI mobile `98`, desktop `91`, and dashboard finalization to `https://5fb1616b.resi-edge-launch.pages.dev`. Closeout checks passed for launch dashboard HTTP `200`, context discipline, PIB guardrails, property identity governance, and `git diff --check`. Added `/Users/mark/Property_Analytics/docs/RESI_EDGE_OPTIMIZATION_MORNING_RUN_PACKET_2026-08-27.md` so the next session starts from the exact closeout state: do not rerun successful properties, keep package invariants fixed, proceed one property at a time, and start with Park on Wurzbach then Retreat at Kedron Village unless Mark changes the order. Boundary: no desktop topper, property-specific Worker fork, WordPress/Kinsta/DNS/forwarding mutation, direct analytics fallback, non-Zaraz analytics ownership, or locked PIB mutation was introduced.

08/27/2026 Data Pond WaterGlass rollback and morning posture addendum: Mark rejected the first broad Venterra WaterGlass CSS pass after live review because the global shell/sidebar/card treatment produced solid white outlines and a wireframe-like feel instead of the fine multi-colored edge effect shown in the reference. Corrective deployment `https://67a467ce.property-analytics.pages.dev` restored the established Data Pond visual framework while preserving the navigation/archive cleanup. Active IA remains: POP Brief and Captain Brief are not active sidebar entries, PIB Builder remains active at `/analysis/pib`, repeated nav headers are removed, Pond footer no longer links to `/marketing`, `/analysis`, `/t7-metrics`, or `/t30-metrics`, and The Dock promotes active destinations including Watchtower, Resi Edge Launch, Pilot Tracker, Captain's Office, PIB Builder, Search Intelligence, GBP Posts, GSC Report, Content Office, VACS, EVS, Employee Photo Audit, and Backup & Export. Morning design packet `/Users/mark/Property_Analytics/docs/DATA_POND_WATERGLASS_MORNING_BRIEF_2026-08-27.md` records the next-pass rule: stay inside the existing Clearwater/Data Pond framework, keep nav calm, prototype only small card-level treatments, and translate the reference as restrained Venterra-colored hairline/gradient edges with subtle inner highlights, not global white borders. Boundary: this is web UI/navigation presentation only; no source data, D1/R2, source-system ticket, API schema, or locked PIB generation/rendering/sending file was changed.

08/26/2026 Resi Edge Balmoral stage, asset-budget correction, and live proof addendum: Balmoral Village (`GA4BV`, `balmoralvillageapts.com`) was promoted into the active production manifest set at `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/balmoralvillageapts-com.manifest.json` from Phase 2 source/evidence data. The first governed stage attempt stopped at `asset_generation_upload_passed` because the mobile hero WebP fallback exceeded the canonical byte cap (`84,532 / 80,000`), before Zaraz setup, route probe, Worker deploy, live route mutation, or dashboard publish. The canonical asset generator `/Users/mark/Property_Analytics/scripts/generate_resi_edge_assets.py` was corrected so quality loops test the exact minimum candidate and fail closed when an asset remains over budget; WebP fallback compression now uses finer bounded steps while preserving AVIF-primary delivery and the byte-budget gate. Balmoral stage rerun passed at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/balmoralvillageapts-com/stage-20260826T205132Z/`, with assets uploaded, Zaraz analytics applied, consent audited, deploy dry-run passed, mobile shell byte forecast `37,732 / 40,000`, consent geometry passed, and `apply_allowed:true`. After Mark approved live apply, Balmoral apply passed `57/57` required gates at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/balmoralvillageapts-com/apply-20260826T210103Z/`, including route interception, live deploy, package health, WordPress/control bypass, no desktop topper, mobile/desktop visual proof, source phone, SEO/schema/identity, Zaraz GA4, consent, Heap interaction-only proof, Ahrefs, Cloudflare analytics, R2/cache proof, PSI mobile `99`, PSI desktop `98`, evidence packet, and dashboard finalization to `https://4028586d.resi-edge-launch.pages.dev`. Boundary: no WordPress/Kinsta mutation, desktop topper, property-specific Worker fork, non-Zaraz analytics ownership, alternate analytics path, or locked PIB mutation occurred.

08/26/2026 Resi Edge Axial apply and dashboard finalization sequencing addendum: Axial Buckhead (`GA4AB`, `axialbuckhead.com`) was promoted into the active production manifest set from Phase 2 source/evidence data, not from the old `pilot-ga4ax.manifest.json` dev artifact. Axial stage passed with current consent, production Heap `286627304`, GA4 stream `Axial Buckhead`, Ahrefs vanity project `10240472`, full drawer nav, same-origin Resi theme fonts, and deploy forecast `37,179 / 40,000` bytes. Live apply evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/axialbuckhead-com/apply-20260826T184200Z/` passed `57/57` gates with route proof, package health, WordPress/control bypass, desktop native pass-through/no desktop topper, mobile shell, source phone, SEO/indexing, Zaraz/GA4/Heap/Ahrefs proof, R2 readback, cache purge, PSI mobile `100`, desktop `90`, and 79 evidence files. A dashboard sequencing issue was corrected in `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py`: future successful applies now write an interim `apply-readout.json` before dashboard finalization, then rewrite the final readout afterward, so the dashboard builder sees the property that just passed without rerunning the completed property process. Dashboard-only refresh/publish produced `https://0d34c344.resi-edge-launch.pages.dev` with Axial represented as optimized proof complete and `3` optimized properties in the summary. Boundary: no desktop topper, property-specific Worker fork, WordPress/Kinsta mutation, direct analytics owner change, alternate analytics/fallback path, or locked PIB mutation was introduced.

08/26/2026 Resi Edge dashboard optimized PSI display addendum: the launch dashboard optimized proof row now shows the highest successful captured PSI sample per strategy as the single executive display number, while the package gate ledger continues to preserve conservative minimum scores for evidence and rollback posture. The snapshot builder reads existing proof packet and stability recheck artifacts; no property apply, PSI rerun, Worker, DNS, forwarding, WordPress/Kinsta, Zaraz, GA4, Ahrefs, R2, cache, or locked PIB mutation was performed. Dashboard-only publish first hit a Cloudflare Pages upload transport `EPIPE`; retry with the current Wrangler release succeeded at `https://82b78962.resi-edge-launch.pages.dev`, and `https://launch.venterrawebops.com/resi-edge/launch` returned `200`. Current optimized display values include Carlyle Place mobile `100` / desktop `99` and Axial Buckhead mobile `100` / desktop `93`.

08/26/2026 Resi Edge dashboard compact rows addendum: the protected launch dashboard property list was simplified into executive scan rows. Each closed row now carries the property name, code, market/home count, launch status, analytics hygiene status, next action, and a compact four-stage PSI journey: Legacy, Staging, Live Vanity, and Optimized. Stage cells show the stage URL plus mobile/desktop PSI scores, while deeper launch steps, monitoring facts, metrics, and trend panels remain behind the drawer. Dashboard-only build passed and was published to Cloudflare Pages deployment `https://2f7b3f6a.resi-edge-launch.pages.dev`; custom host `https://launch.venterrawebops.com/resi-edge/launch` returned `200`. Boundary: dashboard presentation only; no property Worker, DNS, forwarding, WordPress/Kinsta, Zaraz, GA4, Ahrefs, R2, cache, or locked PIB mutation was performed.

08/26/2026 Resi Edge GA4 bootstrap dependency removal addendum: Carlyle Place exposed a canonical analytics assumption rather than a Carlyle-specific problem. Earlier successful sites had a native WordPress/Resi Elements non-network `dataLayer` / `gtag("config", ...)` bootstrap, while Carlyle did not; the Resi Edge proof path was therefore depending on source-site residue even though Zaraz remained the analytics owner. The shared runtime is now `2026-08-26.ga4-datalayer-bootstrap-v1` and supplies the manifest GA4 measurement id into the non-network dataLayer handoff when absent, without direct `gtag.js`, GTM, native Heap, Ahrefs, Resi pixel, desktop topper, or property-specific Worker behavior. Zaraz setup now supports `--force-republish`, and the runner accepts `republished` so canonical config is pushed even when semantic diffing sees no change. Manifest validation and process scenario audit now block non-same-origin Resi Edge font URLs; Carlyle's stale Kinsta font URLs were corrected to same-origin Resi theme paths, and the same manifest normalization was applied to Anatole, District, Vine, Townestone, and Ventana after the batch audit caught their remaining absolute font URLs. Carlyle stage evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/carlyleplacesa-com/stage-20260826T171100Z/` passed, and live apply evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/carlyleplacesa-com/apply-20260826T171204Z/` passed required live proof with runtime health `2026-08-26.ga4-datalayer-bootstrap-v1`, GA4/Zaraz proof, Heap production id `286627304`, WordPress/control bypass, PSI mobile `100`, desktop `93`, and dashboard finalization `https://52fdbc05.resi-edge-launch.pages.dev`. Follow-up process audit evidence passed across `9/9` active production manifests with no baseline failures. Boundary: this is a canonical package repair inside the existing runner/gate path, not a new workflow, fallback analytics layer, WordPress/Kinsta mutation, direct analytics ownership change, or locked PIB mutation.

08/26/2026 Resi Edge canonical byte-margin and dashboard-retry addendum: The District Universal Boulevard exposed a package-size risk during stage when `deploy_bundle_closure_verified` stopped the run at `40,144 / 40,000` forecast bytes. The correction was made only in the shared Resi Edge runtime, now `2026-08-26.mobile-shell-byte-margin-v1`: redundant hidden metadata was removed and the internal analytics bootstrap was compressed while preserving the approved public behavior and proof contract for mobile shell, full drawer nav, consent, Zaraz-owned GA4 `page_view`, Resi Edge events, Heap production id `286627304`, source attribution payloads, and desktop native pass-through. Static validation now protects the compressed analytics implementation shape. The runner's dashboard finalization path now retries only the exact transient Cloudflare Pages upload failure with per-attempt evidence and `--commit-dirty=true`; dashboard-only finalization failures should be recovered by dashboard-only retry, not by rerunning a successful property apply. District final apply evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/thedistrictuniversal-com/apply-20260826T152815Z/` passed `57/57` gates, live health readback, PSI mobile `100`, desktop `91`, and dashboard publish `https://58cca9e2.resi-edge-launch.pages.dev`. Boundary: no desktop topper, property-specific Worker fork, WordPress/Kinsta mutation, non-Zaraz analytics ownership, or locked PIB mutation was introduced.

08/25/2026 Daily GSC indexing action queue addendum: added the governed read-only daily indexing action queue under `/Users/mark/Property_Analytics/Data_Collection/utils/gsc_indexing_actions.py`. The utility reads latest `gsc_url_inspection` rows, resolves property identity through the governed matrix helper, and classifies inspected URLs against the standard migrated Resi page contract: `/`, `/apartments/`, `/features/`, `/amenities/`, `/gallery/`, `/neighborhood/`, `/faqs/`, `/reviews/`, `/contact/`, `/specials/`, and `/about/`. `/Users/mark/Property_Analytics/Data_Collection/orchestration/daily_master_collection.py` now expands daily URL Inspection to `25` URLs per property, prioritizes that standard core page set for migrated/vanity Resi sites, preserves `known_page_paths`, and emits the packet immediately after URL Inspection. `/Users/mark/Property_Analytics/Data_Collection/monitoring/alert_sender.py` now includes the latest GSC Indexing Action Queue in the morning monitoring email and alert snapshot, while preserving the narrower critical core-indexation warning lane. First baseline packet is `/Users/mark/Property_Analytics/reports/gsc_indexing/daily_actions/20260825_115833_gsc_indexing_actions/`, with `92` properties inspected, `325` inspected URLs, `79 / 132` standard pages inspected, `63` standard pages indexed, `53` standard pages missing inspection, and `69` actionable URLs. Current P0 baseline cluster is Calais Midtown standard pages excluded by `noindex` / `BLOCKED_BY_HTTP_HEADER`. Boundary: this capability is read-only and performs no URL submission, sitemap mutation, request-indexing automation, source content edit, Cloudflare change, Resi change, WordPress/Kinsta change, or locked PIB mutation.

08/25/2026 Resi Edge promoted-manifest drift guard addendum: Carlyle Place exposed a draft-promotion process defect before live apply when stale draft fields reached runner validation. The package now makes that failure mode executable instead of relying on operator memory. `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` blocks promoted manifests before plan/stage/apply when they retain draft-only fields, GA4 status is not configured/Zaraz-owned, GA4 stream name is missing or generic for non-lease-up properties, Heap is not production app id `286627304`, required drawer nav labels are missing, leasing URLs are not canonical `online.venterraliving.com` URLs, Ahrefs vanity project id is not numeric/verified, rollback does not declare no WordPress mutation, or rollback script state is not recorded/explicitly `not_yet_recorded`. Added `/Users/mark/Property_Analytics/scripts/audit_resi_edge_rollout_process.py` as a read-only scenario audit that clones manifests in memory, injects known bad states, and fails unless the canonical runner blocks them. Added `/Users/mark/Property_Analytics/scripts/audit_resi_edge_rollout_batch.py` as a production-manifest inventory audit that rejects duplicate active domains/property codes, filename/domain mismatches, stale release-token manifest references, and stale rollout-register manifest references. The runner now requires `batch_inventory_audit_passed` and `process_scenario_audit_passed` after static validation, and `/Users/mark/Property_Analytics/scripts/validate_resi_edge_package_static.mjs` plus `/Users/mark/Property_Analytics/scripts/check_resi_edge_gate_coverage.py` protect those gates. Duplicate active Townestone manifest `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/townestoneat359-com-v2-canary.manifest.json` was retired; Townestone now has one active inventory manifest. The release token default consent version/source now matches shared consent widget contract `compact_shell_pill_v29_2026_08_20`, and `/Users/mark/Property_Analytics/scripts/validate_resi_edge_release_control.py` now fails if token consent defaults lag the contract. Batch audit across production Resi Edge manifests passed `9/9` after stale manifest drift was corrected in Calais, Champions, Harrison, District, Ventana, and Townestone duplicate inventory was retired. Evidence: `/Users/mark/Property_Analytics/reports/resi_edge_performance/process-audit-batch-20260825T013500Z/batch-process-audit-summary.json`. Carlyle passes Python compile, static validation, batch inventory audit, process scenario audit, and non-mutating plan under the hardened guard. Boundary: no live Carlyle Worker route was applied under this correction; earlier Carlyle stage setup had already uploaded assets and applied Zaraz setup, but no live apply was run.

08/25/2026 Resi Edge master runtime sync addendum: read-only live mobile probes confirmed Anatole at Norman serves the current canonical runtime `2026-08-25.explicit-ga4-pageview-v1`, while Calais Midtown, Champions Green, The District Universal Boulevard, The Harrison, The Vine Kyle Parkway, Townestone at 359, and Ventana still serve prior runtime `2026-08-20.shell-payload-optimizer-v1` despite current consent and Heap production id. Added `/Users/mark/Property_Analytics/docs/RESI_EDGE_MASTER_RUNTIME_SYNC_RUN_PACKET_2026-08-26.md` as the 08/26/2026 morning run packet for governed redeploy/readback of those seven already-live optimized properties before starting Carlyle as the next new optimization candidate. The audit boundary is explicit: shared runtime updates do not automatically propagate to deployed Workers, so every property must pass plan, stage, apply with live proof, dashboard finalization, and `data-vtr-package` readback before it is considered current.

08/25/2026 Resi Edge optimization finalization addendum: corrected the Anatole optimization reporting drift and made launch dashboard publication part of the governed apply finalization path. The runner `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` now treats live PSI as a real gate with explicit targets of mobile `98+` and desktop native passthrough `90+`; below-target scored samples use the bounded stabilization retry path, and a final below-target result fails the PSI gate and rolls back. The dashboard snapshot builder `/Users/mark/Property_Analytics/scripts/build_resi_edge_launch_dashboard_snapshot.py` now prefers the latest clean-URL PSI stability recheck by strategy for optimized proof, so Anatole renders optimized PSI as mobile `100` and desktop `98` rather than the preserved old outlier minimum. After all package gates pass, apply finalization now refreshes `/Users/mark/Property_Analytics/apps/web/src/lib/resi-edge-launch/generated-snapshot.ts`, builds the static launch web app with `NEXT_PUBLIC_API_BASE_URL=https://launch.venterrawebops.com` and `NEXT_PUBLIC_AUTH_PRIMARY=magic`, publishes Cloudflare Pages project `resi-edge-launch` through the Keeper-backed Wrangler helper by default, and writes `dashboard/dashboard-finalization.json` under the property apply evidence packet. Dashboard finalization failure blocks the next property but does not roll back a property whose package gates already passed. The apply runner also now writes `phase-timings.json` incrementally and embeds `phase_timings` in success/failure readouts so each optimization post-mortem has exact phase start/end/duration/status rather than reconstructed file-timestamp estimates. The dashboard was refreshed live to Cloudflare Pages deployment `https://89db74d0.resi-edge-launch.pages.dev`; custom host `https://launch.venterrawebops.com/resi-edge/launch` serves the new build chunk containing Anatole optimized proof, mobile PSI `100`, desktop PSI `98`, and runtime `2026-08-25.explicit-ga4-pageview-v1`. Boundary: this did not add a desktop topper, property-specific Worker rebuild, WordPress/admin/control path caching, non-Zaraz analytics ownership, or locked PIB mutation.

08/24/2026 Captain Cloudflare refresh addendum: added a dedicated Cloudflare-owned Captain refresh control plane under `/Users/mark/Property_Analytics/ops/cloudflare/captain-refresh/`. The Worker is configured with a 30-minute Cron Trigger, D1 binding to `pop-brief-db`, R2 binding to `pop-brief-uploads`, and the corporate custom domain `captain-refresh.venterrawebops.com`. D1 migration `/Users/mark/Property_Analytics/apps/api/migrations/0066_create_captain_refresh_tables.sql`, mirrored by `/Users/mark/Property_Analytics/infra/migrations/043_create_captain_refresh_tables.sql`, adds `captain_persona_profiles`, `captain_refresh_runs`, and `captain_office_wall_snapshots`. The Worker reads governed D1 Captain/Awareness/Ops Watch state, creates missing Captain persona defaults, tracks the existing-Captain family composition due date of `09/07/2026`, writes current Office Wall snapshots to D1, and stores JSON snapshot/run evidence under R2 prefix `captains/`. Remote D1 migration was applied to `pop-brief-db`; Worker latest deployed version is `2920b8ec-8bf4-48d2-a208-ae687d327599`; initial corporate Git commit is `d19b96d`. Live validation on 08/24/2026 proved the health/status routes. First scheduled run `captain-refresh-20260824T170013Z-7dcc035fa2e3` succeeded with `7` snapshots before a follow-up Worker correction merged Awareness identities with active Captain support-agent properties; production SQL readback now resolves `94` active Captain properties for the next scheduled run. Added runbook `/Users/mark/Property_Analytics/docs/CAPTAIN_CLOUDFLARE_REFRESH_RUNBOOK_2026-08-24.md` and linked it from README, docs index, Cloudflare ops docs, and the Cloudflare offload plan. Boundary: this moves refresh/orchestration state into Cloudflare but does not grant inward intranet access and does not edit Jira, Confluence, Microsoft 365, source tickets, locked PIB files, or source-system data. External harvesting remains mirror/push or a separately approved Keeper-backed Worker credential lane.

08/24/2026 Captain's Office progressive-disclosure addendum: reworked `/Users/mark/Property_Analytics/apps/web/src/app/captains/captain-office-client.tsx` so the hosted Captain's Office is organized as a compact operator workspace instead of an always-visible vertical page stack. The surface now presents status posture plus five selectable workspaces: Runtime, Watch & Actions, Quarters, Expert Reads, and Lineage. Existing governed data wiring remains intact for Captain Runtime interaction submit/readback, structured response, evidence packets, watch items, routing/actions, Awareness memory posture, candidate memory, Expert Reads, and runtime lineage. Deep-link routes remain compatible: `/captains/<property>`, `/history`, `/watchlist`, `/quarters`, and `/expert-reads` map to their matching workspace. The change reduces one-column gutters by pairing related content horizontally where appropriate: Runtime/Evidence, Watch/Routing, Expert/Authority, and Log/Lineage. Published Cloudflare Pages deployment `https://78961093.property-analytics.pages.dev`; protected user route is `https://pond.venterrawebops.com/captains/<property>`. Verification passed with `npm --prefix apps/web run build`. This was UI composition work only and did not mutate Captain Runtime data, Jira, Confluence, Microsoft 365, Cloudflare D1/R2, source-system tickets, or locked PIB files.

08/24/2026 Captain fleet current-state addendum: brought the Captain Runtime current after the Ops Watch Pond visibility work. Applied reviewed Jira Captain Watch SQL from `/Users/mark/Property_Analytics/reports/captains_log/jira_ticket_watch/jira-captain-watch-20260824-0819/captain-watch-upserts.sql` to remote D1 through Keeper-backed Cloudflare auth, adding/updating `14` Jira watch items and `14` Jira actions across `12` property Captains without mutating Jira or Confluence. Corrected duplicate active identity for The Vine by retiring/superseding legacy GA4-id Captain `505234023` while keeping governed `TX4EK` active: `11` support-agent rows retired, `1` activation memory deprecated, `7` watch rows superseded, and `3` action rows superseded with no deletion. Updated `/Users/mark/Property_Analytics/scripts/audit_captain_readiness.py` so readiness audits use the active remote Captain fleet when it is broader than the latest scoped activation manifest and so Markdown dates render as `MM/DD/YYYY`. Current remote Captain Runtime state is `94` active Captain properties, `1,034` active support-agent rows, and `94` active activation memories. Refreshed audit `/Users/mark/Property_Analytics/reports/captains_log/readiness/captain_readiness_audit_2026-08-24_current.md` reports `94` audited, `91` partial, `3` source-gap, `0` ready, and `0` activation-gap; source-gap properties are Sundara at Spring Cypress (`521906919`), The Vine Kyle Parkway (`TX4EK`), and Monteverde (`TX4MV`). Property identity governance passed with `94` properties, `94` community IDs, and `93` property codes. Remaining readiness work is source freshness/runtime cadence, not activation.

08/24/2026 Ops Watch Pond visibility addendum: added a read-only Data Pond visibility layer over the governed Ops Watch packet system. The generator `/Users/mark/Property_Analytics/scripts/build_ops_watch_pond_snapshot.py` reads a local `ops-watch-packet.json` and emits typed static web data to `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/generated-snapshot.ts`, governed by `/Users/mark/Property_Analytics/apps/web/src/lib/ops-watch/types.ts`. The current snapshot source is `/Users/mark/Property_Analytics/reports/ops_watch/ops-watch-20260824-0819/ops-watch-packet.json`; the human readout remains `/Users/mark/Property_Analytics/reports/ops_watch/ops-watch-20260824-0819/OPS_WATCH_READOUT.md`. The Pond landing page now surfaces Jira/Captain property rows, Critical pressure, pending-vendor rows, property visibility, source-readiness blockers, and top Jira property queue. The Watchtower now exposes an `#ops-watch` section with the current ticket/source posture, priority property queue, source blockers, and assisted-action boundary. Published Cloudflare Pages deployment `https://5382cf5c.property-analytics.pages.dev`; the active protected route is `https://pond.venterrawebops.com/pond`, with Watchtower at `https://pond.venterrawebops.com/watchtower#ops-watch`. Current snapshot counts are `12` Jira issues reviewed, `14` Jira/Captain records, `25` Confluence source signals, `29` total Captain records, `25` properties, `13` Critical records, `5` pending-vendor records, `5` stale `14+` records, and `0` unresolved property records. This is display/readout work only; it does not write Jira, Confluence, Microsoft 365, D1, Captain Runtime, Cloudflare, or locked PIB files. Jira writeback and Captain Runtime publish remain separate reviewed steps.
...
08/24/2026 Jira assisted-handling addendum: extended `/Users/mark/Property_Analytics/docs/JIRA_CAPTAIN_WATCH_RUNBOOK_2026-08-22.md` with an approval-gated assisted Jira handling model. The agent may inspect tickets, classify likely low-friction handling paths, draft exact customer-facing comments and workflow moves, and surface candidates for Mark's final approval. Jira mutation still requires explicit current-conversation approval before posting comments, transitioning, closing, or otherwise changing a ticket. The same update memorialized the Venterra Pricing Link, `https://venterra.atlassian.net/servicedesk/customer/portal/4/create/128`, as the polite routing resource for pricing, concession, system-special, and floor-plan-specific special requests that are not website-owned main hero banner work. First approved example: `MKT-14427` was acknowledged after reporter confirmation and moved to `Done`. Standard customer-facing close is `Thanks, and have a great day!`
...
- VACS current-state reporting should be explicit rather than aspirational:
  ...
  - The Pond now includes VACS drafting inside AI Content Suite over mapped live Site Content sections
  ...
  - `/site-content` is the active operator workspace, while `/vacs` is compatibility access to that same Pond editor
  - `vacs.venterradev.com` is superseded as an operator destination for this Pond content lane
...
### 08/22/2026 Resi Edge Heap Hygiene Evidence And Optimization Dashboard Gate

- The 20-property post-launch optimization prep lane now has a separate read-only Heap hygiene evidence builder at `/Users/mark/Property_Analytics/scripts/build_resi_edge_heap_hygiene_evidence.py`.
- The checker distinguishes live launch completion from optimization readiness by checking for old/direct Heap `676880719`, expected production Heap `286627304`, dual IDs, and `HEAP_JS_DEBUG=true` in source, plus optional browser-load samples for real Heap network requests.
- Latest packet `/Users/mark/Property_Analytics/reports/resi_edge_performance/heap-hygiene/heap-hygiene-20260823T021805Z/` records `0/20` clean for optimization, old Heap on `20/20`, expected production Heap on `19/20`, dual IDs on `19/20`, debug true on `20/20`, and browser samples with old Heap activity on `3/3`.
- The launch dashboard snapshot generator and UI now expose this as an `Analytics Hygiene`/`Analytics Clean` gate. Public move/redirect status can remain green while optimization is visibly blocked until Resi removes duplicate Heap/debug output.
- This is a dashboard/reporting capability only. It does not mutate DNS, Cloudflare, Workers, WordPress/Kinsta, Zaraz, GA4, Ahrefs, R2, cache, Captain, Data Pond, or live domains.

### 08/25/2026 Resi Edge Consent Readiness And Plan-Mode Classification

- The Anatole optimization test plan exposed an already-known lifecycle mismatch: stage/apply had governed Zaraz consent setup, but `plan` still treated a not-yet-configured zone as a failed consent gate instead of an anticipated setup action.
- Mark approved correcting consent at the source. The governed Zaraz consent package was applied across the first `20` launched vanity domains; `19` domains were updated, Anatole was already unchanged after the canary fix, and the independent follow-up audit passed `20/20`.
- Evidence lives at `/Users/mark/Property_Analytics/reports/cloudflare_zaraz/consent_management/first20_20260825_zaraz_consent_apply.json` and `/Users/mark/Property_Analytics/reports/cloudflare_zaraz/consent_management/first20_20260825_zaraz_consent_audit_after_apply.json`.
- `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` now classifies missing consent in `plan` as `not_run` with explicit detail that stage will apply and re-audit the governed package before route probe or Worker deploy. Stage/apply continue to fail closed if post-setup consent proof fails.
- This correction strengthens the existing Resi Edge launch model without introducing a parallel path: no property Worker deploy, route mutation, DNS/forwarding change, WordPress/Kinsta mutation, R2/cache mutation, GA4/Ahrefs mutation, dashboard deploy, or locked PIB mutation was performed.

### 08/25/2026 Resi Edge Native GTM Sanitizer And Anatole GA4 Stop

- The Anatole optimization canary confirmed that the newly launched native WordPress desktop page still served direct GTM. The first live apply stopped at direct native analytics blockers and rolled back.
- The shared Resi Edge runtime now includes a public-page-only GTM sanitizer, versioned as `2026-08-25.desktop-native-gtm-sanitizer-v1`. It removes native GTM bootstrap snippets from desktop/native-continuation HTML after origin fetch, while control/admin paths continue to bypass all optimization and caching.
- Static validation now protects this sanitizer so future package edits cannot remove it silently. Anatole stage passed, then the live retry proved the intended fix: no mobile or desktop direct native analytics blockers, desktop native rendering passed, event bridge passed, consent passed, source phone and SEO checks passed, R2/cache checks passed.
- The live retry then stopped and rolled back at GA4 realtime proof. Browser-side evidence showed Zaraz, Ahrefs, and Heap behavior present, but the GA4 Admin realtime query returned no `page_view`, no `session_start`, and no expected stream row. This is the next blocked evidence item; PSI was not run after the failed analytics gate.

### 08/25/2026 Resi Edge Explicit GA4 Pageview Proof

- The GA4 stop was traced to an implicit-event contract flaw: the Resi Edge runtime emitted the Resi custom view event (`edge_desktop_native_view` / `edge_mobile_topper_view`) and relied on Zaraz's system `Pageview` signal to create GA4 `page_view` and `session_start` evidence. That dependency was not reliable enough for the rollout.
- The shared runtime is now `2026-08-25.explicit-ga4-pageview-v1`. It emits a package-owned `page_view` event with page title/location/referrer and governed property identifiers, then emits the Resi custom view event. It also exposes a short `__vtrEdgeQueue` / `vtrEdgeFlush` path so events are not lost if Zaraz is slightly late.
- `/Users/mark/Property_Analytics/scripts/apply_resi_zaraz_analytics_package.py` now adds a `ResiEdgePageview` trigger and maps `page_view` to the GA4 pageview action, while excluding `page_view` from the generic custom-event action. Static validation protects this runtime/Zaraz pairing.
- Anatole live apply then passed all `55/55` gates with no rollback. Evidence lives at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/anatoleatnorman-com/apply-20260825T154926Z/`. GA4 realtime proved `first_visit`, `page_view`, and `session_start` on stream `Anatole at Norman`; browser proof showed `dataLayer` events `page_view` and `edge_desktop_native_view`, Zaraz requests, Heap production app id `286627304`, no unexpected Heap ids, no direct `gtag`, and no passive Heap load. The initial PSI gate packet contained an outlier desktop exact sample; current dashboard PSI is governed by the later PSI correction below.
- `/Users/mark/Property_Analytics/scripts/build_resi_edge_launch_dashboard_snapshot.py` now reads passed optimization apply packets and exposes an optimized proof state to the launch dashboard. Anatole now renders as optimized proof complete with its live evidence packet and runtime version instead of remaining a future placeholder.
- This is the current canonical package baseline for the remaining property optimization run. It preserves the established guardrails: no desktop topper, no property-specific Worker rebuild, no WordPress/admin/control path optimization or caching, and Zaraz-owned analytics.

### 08/25/2026 Resi Edge PSI Drift Correction

- A follow-up review found that the first optimized-dashboard pass had drifted by reading the old apply packet's `desktop_min_score` (`67`) as the optimized desktop score. That packet contained one anomalous desktop exact run with high TBT, while later clean-URL stability evidence proved desktop in the high `90s`.
- Read-only Anatole PSI stability evidence now shows mobile `100`, `100`, `100` and desktop `98`, `98`, `93` across exact/fresh samples. Dashboard optimized proof now renders Anatole as mobile `100`, desktop `98`; the old `67` remains preserved only as historical evidence in the old apply packet.

### 08/26/2026 Resi Edge GA4/Zaraz Proof Invariant Repair

- Calais runtime sync exposed a proof-contract flaw in `/Users/mark/Property_Analytics/scripts/smoke_live_analytics.py`: the smoke required GA4 Realtime `session_start` as a live apply blocker even though `session_start` is an automatic GA4 reporting event, not a package-owned Resi Edge event.
- The repair stays inside the existing package/gate path. The canonical analytics proof now blocks on the invariant shared-template behavior: explicit package-owned `page_view` in browser `dataLayer`, Zaraz transport, and the manifest GA4 measurement ID present in the Zaraz bootstrap. Realtime event/stream/minute output remains diagnostic evidence and no longer redefines the package contract.
- Added regression coverage in `/Users/mark/Property_Analytics/tests/test_smoke_live_analytics_ga4_gate.py` for the Calais-style case where `page_view` is present but `session_start` is absent, and for avoiding stream/pageview inference from separate realtime totals.
- Validation passed with the repaired gate and no live mutation: `python3 -m pytest tests/test_smoke_live_analytics_ga4_gate.py`, `python3 scripts/check_resi_edge_gate_coverage.py`, `node scripts/validate_resi_edge_package_static.mjs --manifest config/portfolio_resi_edge_stabilization/calaismidtownapartments-com.manifest.json`, and `bash scripts/check_pib_guardrails.sh`. No new certification layer, property-specific branch, fallback event, Worker/runtime change, or rollout continuation was introduced.
- The underlying gate policy was also corrected. `/Users/mark/Property_Analytics/scripts/run_resi_edge_upgrade.py` no longer treats desktop native passthrough PSI as recorded-only. It now enforces mobile `98+` and desktop `90+`, retries below-target scored samples within the bounded stabilization policy, and fails/rolls back when final PSI remains below target.
- `/Users/mark/Property_Analytics/scripts/validate_resi_edge_package_static.mjs` now guards against reintroducing the recorded-only desktop PSI policy. `/Users/mark/Property_Analytics/scripts/build_resi_edge_launch_dashboard_snapshot.py` now sources optimized PSI from the latest clean-URL stability recheck by strategy, using file modification time rather than folder-name sorting.

### 08/26/2026 Resi Edge Dashboard Lead Result Rows

- The protected launch dashboard now treats compact property result rows as the primary executive/operator view. Each property row leads with name, status, next action, and the four-stage performance journey from legacy to optimized proof.
- Supporting portfolio context remains available to Steward/Admin users but is intentionally below the alphabetized property rows: command summary, current portfolio truth, work ahead, market mix, portfolio pipeline, benchmark summary, and scrutiny overview. Non-stewards see only the compact property rows with no drawers. Pending optimization properties are visually ghosted with a clear banner. This is a dashboard presentation change only, with no live property mutation.

### 08/26/2026 Boulevard At Lakeside Optimization Closeout

- Boulevard at Lakeside (`OK4BL`, `blvdatlakeside.com`) completed the governed Resi Edge optimization sequence through `plan`, `stage`, and `apply --require-live-proof`. The live packet `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/blvdatlakeside-com/apply-20260826T213732Z/` passed all required gates, wrote `79` evidence files, deployed Worker route `blvdatlakeside.com/*`, and recorded optimized PSI mobile `100` / desktop `97`.
- The run stayed on the current canonical package: no desktop topper, no property-specific Worker rebuild, no WordPress/admin/control-path optimization, no direct analytics fallback, and no locked PIB mutation. Dashboard finalization published deployment `https://c57d4a59.resi-edge-launch.pages.dev` for `launch.venterrawebops.com`.

### 08/26/2026 Launch Dashboard Header-Card Row Correction

- The protected launch dashboard row layout was corrected so property name, code, market/home count, QA status, launch date, analytics hygiene, and drawer chevron sit in a full-width row header. The four proof stages now use the full row width and restore desktop arrows between `Legacy`, `Staging`, `Live Vanity`, and `Optimized`.
- Closed executive rows no longer show the `Next Action` paragraph, and properties pending optimization are greyed out at the full-row level while retaining the pending optimized target marker. Dashboard-only builds passed and were published to Cloudflare Pages deployments `https://d9faabf2.resi-edge-launch.pages.dev` and `https://148164ac.resi-edge-launch.pages.dev`; the custom host `https://launch.venterrawebops.com/resi-edge/launch` returned HTTP `200`. Boundary: dashboard presentation only; no property Worker, DNS, forwarding, WordPress/Kinsta, Zaraz, GA4, Ahrefs, R2, cache, or locked PIB mutation occurred.

### 08/26/2026 Canton Mill Lofts Stage-Ready State

- Canton Mill Lofts (`GA4CM`, `livecantonmill.com`) now has an active promoted Resi Edge manifest at `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/livecantonmill-com.manifest.json`, built from the prepared Phase 2 draft plus current live source proof captured on 08/26/2026.
- The governed `plan` and `stage` sequence passed. Stage evidence lives at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/livecantonmill-com/stage-20260826T223733Z/` and includes R2 asset upload, Zaraz analytics package upserts, Zaraz consent ready proof, Ahrefs existing-project confirmation, deploy-bundle dry-run, consent geometry proof, and reference replay.
- Mark approved live apply. Apply evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/livecantonmill-com/apply-20260826T224216Z/` passed all required gates, wrote `82` evidence files, and deployed Worker route `livecantonmill.com/*` with version id `9e7856fd-cb58-41d3-88a2-4f6c62721938`.
- Canton Mill live proof preserved the launch contract: no desktop topper, no property-specific Worker rebuild, no WordPress/admin/control-path optimization, Zaraz-owned analytics, production Heap app id `286627304`, GA4 measurement `G-3DCPYZ87WV`, and source-coded phone/CTA attribution. PSI gate passed with mobile minimum `100` and desktop minimum `95`; dashboard optimized proof records the highest successful samples as mobile `100` and desktop `99`.
- Dashboard finalization passed and published Cloudflare Pages deployment `https://e56399fd.resi-edge-launch.pages.dev`; the launch snapshot now reports `6/20` optimized.

### 08/26/2026 Creekside Optimization Closeout

- Creekside (`OK4CS`, `creeksideapt.com`) completed the governed Resi Edge sequence from promoted active manifest through `plan`, `stage`, and `apply --require-live-proof`. The active manifest is `/Users/mark/Property_Analytics/config/portfolio_resi_edge_stabilization/creeksideapt-com.manifest.json`.
- The live source review used `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/creeksideapt-com/source-scrape-prep/creeksideapt.com_20260826T230847Z.html` and confirmed the current vanity canonical, index/follow robots, production Heap app id `286627304`, GA4 measurement `G-211V5BP4SE`, full drawer navigation, reviews `4.3` / `289`, and Kingsley award. Since no active homepage promo was present, Creekside remains `mobile_shell.promo.present=false` with the canonical no-active-promo source marker.
- Stage evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/creeksideapt-com/stage-20260826T232346Z/` passed with asset upload, Zaraz analytics package, consent audit, Ahrefs project confirmation, deploy-bundle dry-run, consent geometry proof, and reference replay.
- Live apply evidence at `/Users/mark/Property_Analytics/reports/resi_edge_performance/08-09-2026/creeksideapt-com/apply-20260826T232518Z/` passed `57/57` required gates, wrote `85` evidence files, and did not require rollback. Proof covered Cloudflare route interception, package health, WordPress control-path bypass, mobile shell contract, desktop native passthrough with no desktop topper, Zaraz consent, GA4/Zaraz, Heap/Contentsquare interaction-only behavior, Ahrefs, Cloudflare analytics state, source-coded phone, SEO/GSC, R2 readback, cache purge, Captain/Data Pond evidence, and final dashboard publication.
- PSI proof recorded mobile minimum `100` and desktop minimum `98`. Dashboard finalization published Cloudflare Pages deployment `https://0743f80b.resi-edge-launch.pages.dev` and the launch snapshot now reports `7/20` optimized.

### 09/01/2026 Resi FAQ Captain Review Job Board

- The Resi FAQ quality review now has a reusable Captain Office assignment path instead of being managed only through an approval spreadsheet. `/Users/mark/Property_Analytics/scripts/assign_resi_faq_captain_review_jobs.py` reads the simple FAQ review workbook, resolves governed property identities, builds one property-level Captain assignment per community, and emits local audit artifacts plus remote Captain Office actions/watch items.
- The first batch used `/Users/mark/Property_Analytics/outputs/resi-faq-captain-review-20260901/resi_faq_captain_review_simple_20260901.xlsx` and wrote the audit packet `/Users/mark/Property_Analytics/reports/captains_log/faq_review_jobs/20260901T222259Z/`.
- Remote Captain Office initially confirmed `92` open `captain_actions` for action key `resi_faq_review_20260901` and `92` open `captain_watch_items` for watch key `resi_faq_review_due_20260901`, due 09/02/2026 at 12:00 PM CT. Mark then rejected waiting for the deadline and asked for immediate execution.
- Immediate execution escalated the batch to critical, woke the `navigator_watch` content/search Captain lane, and used six parallel review workers plus local fallback review. Merge runner `/Users/mark/Property_Analytics/outputs/resi-faq-captain-review-20260901/merge_captain_review_outputs.mjs` exported reviewed workbook `/Users/mark/Property_Analytics/outputs/resi-faq-captain-review-20260901/resi_faq_captain_review_simple_captain_reviewed_20260901.xlsx`.
- Final review workload result: `2,259` FAQ rows updated across `92` properties, `2,110` rows marked `Captain Reviewed`, `144` rows marked `Needs Manager Fact Approval`, and `5` rows marked `Blocked - Source Gap`. QA found no unmatched FAQ IDs, no formula errors, clean workbook zip integrity, and no remaining reviewed-field defects for `poor area`, HTML leakage, `Top amenities include No`, or actual `to to` typo.
- Remote Captain Office was updated to `92` `done` actions and `92` `resolved` watch items for the batch. This creates a visible job-board/checklist lane and a completed review artifact for managers, but human read receipts still require a separate notification or acknowledgement integration.
- Boundary: this capability does not publish FAQ changes to Resi, WordPress, vanity domains, or legacy URLs and does not bypass Content Manager or SEO Manager approval.

### 09/04/2026 Portfolio Organic Non-Brand Search Reporting

- The governed ad hoc executive report system now includes an `organic_nonbrand_search_terms` lane for Andrew Foresi-style portfolio search reporting. It extends the existing Outlook-safe ad hoc packet path rather than creating a one-off renderer.
- The report joins GA4 owned-site performance with GSC search-demand facts, classifies GSC query rows per property, and produces strict-discovery non-brand rollups by `encasa_region`. The workbook contains the full top 10 terms per group plus group summary, channel mix, monthly trend, brand/non-brand mix, intent clusters, property opportunities, and core source coverage. The Andrew packet is GA4/GSC-only for performance metrics so it does not mix certified owned-site/search-term facts with advisory or partial snapshots.
- Keeper-backed backfill helpers now support this reporting lane: `/Users/mark/Property_Analytics/scripts/backfill_portfolio_gsc_queries.py` for query rows, `/Users/mark/Property_Analytics/scripts/backfill_portfolio_gsc_daily.py` for daily GSC performance, and `/Users/mark/Property_Analytics/scripts/backfill_portfolio_gbp.py` for separately auditable GBP daily/monthly evidence. They write auditable run packets under `/Users/mark/Property_Analytics/reports/adhoc_executive/andrew_foresi_search_backfill/`.
- Source disposition: GA4 and GSC are included; Google Ads is excluded because it is paid-media context and historical 2025 mapped activity was not portfolio-representative; GBP is excluded from the core organic term report because it is profile/local demand; Ahrefs and DataForSEO are excluded because stored coverage is limited snapshot/credential-dependent data, not a backfilled portfolio performance source for this window.
- Latest executive packet: `/Users/mark/Property_Analytics/reports/adhoc_executive/organic_nonbrand_search_terms/20260904_125828_andrew-foresi-portfolio-non-branded-organic-search-performance-by-group/`. It passed Outlook validation and was not emailed.
- Boundary: this is reporting/backfill/Data Pond work only. It does not mutate PIB locked files, Cloudflare, DNS, Workers, WordPress/Kinsta, Zaraz, GA4 configuration, Ahrefs configuration, R2/cache, Captain, or live domains.
