...
- the first additional Data Warehouse replacement tracks are now active and documented in `/Users/mark/Property_Analytics/docs/DATA_WAREHOUSE_REPLACEMENT_TRACKS_2026-05-26.md`: `/Users/mark/Property_Analytics/apps/api/scripts/guest_cards_to_d1.py` now prefers DW direct guest-card rows where present, `/Users/mark/Property_Analytics/scripts/supply_property_operating_metrics_from_data_warehouse.mjs` supplies `property_operating_metrics` from `dw_read.leasingstatistics_daily_bv`, `/Users/mark/Property_Analytics/scripts/supply_property_metadata_from_data_warehouse.mjs` supplies `property_metadata_dw_direct` from `dw_read.property_bv`, and `/Users/mark/Property_Analytics/scripts/audit_manual_source_replacements.mjs` tracks replacement gates for legacy/manual CSV paths through `/Users/mark/Property_Analytics/config/manual_source_replacement_manifest.json`
- operating-metrics identity gaps are now resolved through `/Users/mark/Property_Analytics/config/data_warehouse_property_code_resolution.json`; `FL4CA`, `FL4P9`, `TX4FP`, and `TX4PW` are classified as governed exclusions from canonical property operating metrics because they appear in `dw_read.leasingstatistics_daily_bv` but not in the checked DW property identity/name views or the governed property identity matrix
- Watchtower now exposes Data Warehouse replacement posture through `/v1/health/status` and the `/watchtower` web surface, while `/Users/mark/Property_Analytics/scripts/generate_data_warehouse_replacement_review.mjs` generates repeatable review packets for metadata deltas, operating baselines, governed exclusions, and lifecycle count checks for pre-live properties not expected in DW operating metrics
...
- the next usability refinement step is now also present:
  - page cards now surface visual cues first through `spec_screenshot` previews or structural mini-maps
