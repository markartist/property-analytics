...
The Pond landing page, Watchtower, Captain's Office Ticket Wall, and Commodore's Bridge consume this generated snapshot. The visible app layer is read-only and should show source pressure, Captain property rows, source blockers, assisted-action posture, Captain Ticket Care queues, and regional Commodore rollups. It must not become a Jira writeback, Captain Runtime publish, Confluence edit, Microsoft 365 action, Cloudflare action, or D1 mutation surface.

09/04/2026 Commodore Bridge note: `scripts/build_ops_watch_pond_snapshot.py` now emits `commodoreBridge` in the generated Pond snapshot. The bridge groups Ticket Care pressure by the governed property identity matrix region, identifies shared pattern candidates, packages regional escalation candidates, and links each pressure point back to the responsible Captain office. The governing model is `/Users/mark/Property_Analytics/docs/COMMODORE_BRIDGE_OPERATING_MODEL_2026-09-04.md`. The Pond route is `/commodores`; the protected hosted route is `https://pond.venterrawebops.com/commodores`.

09/04/2026 Commodore activation note: `config/commodore_roster.json` is the governed source for active Commodore names, call signs, standing orders, and cadence. Ops Watch snapshot generation should preserve `commodoreBridge.roster`, `activeCommodoreCount`, and per-region `standingOrders` so regional managers and system personas receive the same orders from the same source.

09/04/2026 Ticket Care note: `scripts/build_ops_watch_pond_snapshot.py` now emits `ticketCare` in the generated Pond snapshot. Each Jira Captain record receives care flags, blocker owner, evidence requirements, customer-promise guidance, Captain stance, and recommended action. Property queues roll those records into the Captain's Office `Ticket wall` workspace. The governing SOP is `/Users/mark/Property_Analytics/docs/CAPTAIN_TICKET_CARE_SOP_2026-09-04.md`. Hosted Pond deployment `https://63acf45d.property-analytics.pages.dev` carries this update; the protected route remains `https://pond.venterrawebops.com/captains/<property>`.
