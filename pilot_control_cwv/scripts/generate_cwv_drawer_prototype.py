...
OUTPUT_PATH = ROOT / "pilot_control_cwv" / "reports" / f"CWV_Drawer_Prototype_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
...
def add_rollup_row(
    ...
    ws.cell(start_row, 1, title).font = Font(size=10, bold=True, color=TEXT_BLACK)
...
def build_workbook() -> None:
    ...
    ws["A1"] = "CWV Drawer Prototype"
    ws["A1"].font = Font(size=18, bold=True, color=TEXT_BLACK)
    ws.merge_cells("A2:L2")
    ws["A2"] = "One KPI group, two source rollups, detail hidden by default. Use the + control on the left margin to expand."
    ws["A2"].font = Font(size=10, color=MUTED_TEXT)

    row = 4
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=12)
    ws.cell(row, 1, "Core Web Vitals").font = Font(size=14, bold=True, color=TEXT_BLACK)
    ws.cell(row, 1).fill = PatternFill("solid", fgColor=SECTION_FILL)
    row += 2

    for r in range(row, row + 10):
        for c in range(1, 13):
            ws.cell(r, c).fill = PatternFill("solid", fgColor=CARD_FILL)
    ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=9)
    ws.cell(row, 1, "Core Web Vitals   [+] Expand details").font = Font(size=13, bold=True, color=TEXT_BLACK)
    ws.cell(row, 1).fill = PatternFill("solid", fgColor=TITLE_FILL)
    ws.merge_cells(start_row=row, start_column=10, end_row=row, end_column=12)
    ws.cell(row, 10, "5 pilot / 5 sister avg").font = Font(size=9, bold=True, color=MUTED_TEXT)
    ws.cell(row, 10).fill = PatternFill("solid", fgColor=ACTION_FILL)
    ...
    data_row = 1
    data_row = add_rollup_row(ws, data_ws, row + 1, "PSI Avg", dates, psi_pilot_avg, psi_sister_avg, PSI_BASELINE, PSI_FLOOR, data_row)
    data_row = add_rollup_row(ws, data_ws, row + 5, "GTMetrix Avg", dates, gtm_pilot_avg, gtm_sister_avg, GTM_BASELINE, GTM_FLOOR, data_row)
    detail_start = row + 10

    current_row = detail_start
    ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=12)
    ws.cell(current_row, 1, "PSI Details").font = Font(size=11, bold=True, color=TEXT_BLACK)
    ws.cell(current_row, 1).fill = PatternFill("solid", fgColor=SECTION_FILL)
    current_row += 1
    ...
    ws.merge_cells(start_row=current_row, start_column=1, end_row=current_row, end_column=12)
    ws.cell(current_row, 1, "GTMetrix Details").font = Font(size=11, bold=True, color=TEXT_BLACK)
    ws.cell(current_row, 1).fill = PatternFill("solid", fgColor=SECTION_FILL)
    current_row += 1
