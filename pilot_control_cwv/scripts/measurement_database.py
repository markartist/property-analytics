...
def replace_measurement_workbook_rows(conn: sqlite3.Connection) -> None:
    conn.execute(
        "DELETE FROM measurement_daily_raw_values WHERE source_file LIKE ?",
        ("%/Measurement_Dashboard%.xlsx",),
    )
    conn.execute(
        "DELETE FROM measurement_daily_metrics WHERE source_file LIKE ?",
        ("%/Measurement_Dashboard%.xlsx",),
    )
    conn.commit()
