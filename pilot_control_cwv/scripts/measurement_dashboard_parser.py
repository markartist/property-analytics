...
from measurement_database import (
    ...
    replace_measurement_source_rows,
...
MEASUREMENT_PATH = Path(
    "/Users/mark/Library/CloudStorage/OneDrive-VenterraRealty(Canada)Inc/Guest_Card_Reports/Measurement_Dashboard_1.1.xlsx"
)
...
def _workbook():
    return load_workbook(MEASUREMENT_PATH, read_only=True, data_only=True)
...
def parse_measurement_daily_records() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    wb = _workbook()
...
def ingest_measurement_workbook(conn) -> tuple[int, int]:
    ensure_measurement_tables(conn)
    raw_records, metric_records = parse_measurement_daily_records()
    replace_measurement_source_rows(conn, str(MEASUREMENT_PATH))
...
def latest_heap_status() -> HeapStatus:
    ...
    return HeapStatus(latest_sheet=latest_sheet, latest_date_label=latest_date_label, today_present=today_present)
