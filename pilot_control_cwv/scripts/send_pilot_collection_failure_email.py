...
def send_failure_alert(stage: str, report_date: str, details: str) -> bool:
    ...
    subject = f"CRITICAL: Consolidated Morning Failure Alert - Pilot {stage} - {report_display_date}"
    html_body = f"""
    <html>
    ...
    sender.send_email_with_tracking(
        subject=subject,
        html_body=html_body,
        plain_text=f"Fresh pilot morning data unavailable for {report_display_date}.\nStage: {stage}\n\n{details}",
        recipients=[
            "mlaufhutte@venterraliving.com",
        ],
        log_path=log_path,
    )
    print(f"✅ Pilot collection failure alert sent for {report_display_date} ({stage})")
    return True
