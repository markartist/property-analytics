...
PY
    ...
    if [ "$TWIN_GT_EXIT" -eq 2 ] || [ "$TWIN_GT_EXIT" -eq 3 ]; then
      send_failure_alert \
        "Twin GTMetrix" \
        "Twin GTMetrix could not produce a complete fresh same-day cohort for ${RUN_DATE} due to GTMetrix rate/credit limits. CSV attachments would otherwise contain blank twin GT values."
      return 1
    fi
  ...
  send_failure_alert \
    "Twin GTMetrix" \
    "Twin GTMetrix could not produce a complete fresh cohort for ${RUN_DATE} after ${MAX_TWIN_GT_ATTEMPTS} attempts. The consolidated roundup attachments are configured to include twin GT values, so export delivery was blocked."
  return 1
}
...
run_homepage_evidence_until_fresh() {
  ...
  send_failure_alert \
    ...
    "Homepage audit evidence collection failed for ${RUN_DATE} after ${MAX_HOMEPAGE_ATTEMPTS} remediation attempts. Fresh GTMetrix and PSI data were already collected, but downstream pilot artifacts were blocked until homepage evidence succeeds."
  return 1
}

{
  ...
  echo "[9/12] Generating merged pilot evaluation..."
  ...
  echo "[10/12] Generating pilot roundup..."
  ...
  echo "[11/12] Handling pilot roundup notification..."
  ...
  echo "[12/12] Pilot morning workflow complete."
  echo
